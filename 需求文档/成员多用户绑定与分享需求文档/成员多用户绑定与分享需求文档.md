...
成员远程邀请新增时，服务端会根据邮箱或手机号查找目标用户，并在邀请记录中写入 `target_user`。当前发现同一手机号在不同 App bundle 下绑定了不同用户时，邀请接口可能匹配到错误 App 的用户。
...
邮箱查询也必须遵守同一规则。若邮箱身份存储在 `SocialIdentity(provider=email)`，查询条件必须包含 `bundle_id`；若当前只使用 `User.email` 查询，则匹配到用户后仍必须确认该用户在当前 `bundle_id` 下存在有效账号身份、设备会话或登录身份，不能只凭全局邮箱跨 App 命中。
...
5. 邮箱先 trim/lowercase，再查询当前 bundle 下的邮箱身份；如果需要兼容 `User.email`，必须追加当前 bundle 身份校验。
...
| `SparkService/medical/services/member_invite_service.py` | `resolve_user_by_contact` 增加 `bundle_id` 参数，手机号和邮箱查询按 bundle 过滤 |
...
4. 同一邮箱跨 bundle 绑定不同用户时，邮箱邀请只匹配当前 bundle 用户。
5. 当前 bundle 下没有对应邮箱身份，但其他 bundle 有该邮箱时，不返回其他 bundle 用户，邀请可继续走邮件直投。
6. 重复发送邀请时，不复用其他 `target_user_id`、其他 `role` 或其他规范化联系方式的 pending 邀请。
7. 响应日志和接口响应中的 `normalized_phone` 正确，敏感日志脱敏。
...
3. 邮箱邀请也按当前认证 bundle 隔离，不跨 App 匹配全局用户。
