...
第二阶段不推翻第一阶段的“成员多用户绑定 + 永久绑定 + 二维码快速分享”闭环，而是在一期稳定后补齐远程邀请、共享治理、权限精细化、多资源分享，以及部分需要较大架构调整的收敛项。
...
本项目按全新项目处理，第二阶段不需要考虑历史数据迁移、旧字段回填或旧接口兼容。以下事项不阻塞第一阶段验收，统一纳入第二阶段：

| 条目 | 原因 | 第二阶段处理方式 |
| --- | --- | --- |
| 删除 `Member.relationship` | 需要全链路 API/客户端模型调整 | 二期按最终模型直接实现，关系只保留在 `UserMemberBinding.relationship`，不做历史数据迁移 |
| 服务端权限入口统一 | 涉及医疗、文件、workflow 多模块重构 | 二期统一 permission service，逐步替换散落判断 |
| `MemberDetail` 删除/解绑收敛 | 需要 UseCase 层调整 | 二期把删除/解绑语义收进 `ManageMemberBindingUseCase` |
| Share 公共 Provider 抽象 | 属于结构性重构 | 二期抽 `ShareResource` / `SharePayloadProvider`，支持多资源分享 |
| BLE 降级为占位实现或拆 adapter | 产品若坚持一期仅二维码，可再拆 adapter | 二期决定正式接入 BLE 或保留占位，补真机验证 |
| 新增成员页增加“接收附近分享”入口 | 需要和近场接收状态、扫码入口统一设计 | 二期在新增成员页增加附近接收入口 |
...
1. 不在现场时也能邀请别人绑定成员。
2. 绑定后可管理共享用户、关系、权限和解绑。
3. 分享能力从“成员”扩展到更多资源。
4. 附近设备从可用升级为稳定、可观测、可诊断。
5. 收敛一期为快速闭环留下的架构债务。
...
1. 分享人选择成员，打开公共分享页。
2. 选择“手机号/邮箱”或“App 内通知”。
3. 输入目标账号标识。
4. 服务端校验目标用户是否存在、是否已经绑定该成员。
5. 创建远程邀请记录并发送通知。
6. 被邀请用户在通知中心或首页待处理入口看到邀请。
7. 点击邀请后进入新增成员页的“绑定成员”模式。
8. 选择与成员的关系并确认绑定。

第二阶段可以新增邀请记录表，但必须和第一阶段的无表 `share_ticket` 区分：
...
MemberShareInvite
  ...
  inviter_user
  target_user
  target_contact       # 手机号/邮箱脱敏存储或 hash
...
- 第一阶段面对面二维码/附近设备仍不需要 `MemberShareInvite`。
- 第二阶段远程定向邀请需要状态管理，因此可以单独建表。
- 邀请记录只管理远程邀请生命周期；真正授权仍以 `UserMemberBinding` 为准。

### 16.3 邀请通知与待处理入口

客户端新增入口：

- 首页成员区可显示“待处理成员邀请”轻提示。
- 通知中心展示“某某邀请你绑定成员”。
- 账号登录完成后拉取待处理邀请。
...
- 当前用户永远可以解绑自己。
- owner 可以移除其他绑定用户。
- owner 可以设置 admin/viewer。
- admin 是否可以移除 viewer 需要产品确认。
- 删除成员档案必须 owner 且只有一个 active binding，或者走更严格的二次确认。
...
```json
{
  ...
  "is_self": false,
  "invited_by": 3,
  "bound_at": "2026-05-24T10:00:00Z"
}
```

### 16.5 权限精细化

第一阶段 active binding 用户拥有成员医疗资料完整读写权限。第二阶段可以引入更细权限，但必须产品确认后再启用。
...
- 不允许客户端自行根据 role 判断权限后直接放行。
- 服务端必须返回能力位，例如 `can_upload_report`、`can_save_ai_result`、`can_manage_bindings`。
- 所有 workflow 继续走统一权限服务，不能重新分散判断。

客户端要求：

- UI 展示以能力位为准。
- role 只作为展示和说明，不作为最终授权依据。
- 权限不足时展示业务错误，不触发登出。

### 16.6 多资源分享

公共分享页第二阶段要从“成员分享”升级为通用资源分享。
...
- 成员分享仍是绑定成员。
- 报告/病历分享不能默认让对方绑定整个成员，除非用户明确选择。
- 医疗资源分享应区分“只分享该资源”和“邀请绑定成员”。
- 第二阶段需要新增资源级权限模型或短期访问票据，不能复用成员绑定直接放大权限。

### 16.7 附近设备增强

第一阶段如产品确认只做二维码闭环，则 BLE 允许降级为占位实现：

- 保留 `NearbyShareTransport` 接口。
- 暂不合并完整 `NearbyShareBLEAdapter`。
- 分享 Sheet 中可隐藏“附近设备”，或展示“即将支持”。
- 不影响二维码分享、扫码绑定和医疗权限闭环。

若第一阶段 BLE 已合并，第二阶段需要补强：
...
- 发现设备排序优化。
- 设备名脱敏。
- DEBUG/Release UUID 隔离。
- 蓝牙权限引导。
- 真机诊断日志。

如果第一阶段 BLE 只是占位，第二阶段再正式接入 CoreBluetooth：

- 分享端作为 Central 扫描接收端。
- 接收端作为 Peripheral 广播可接收状态。
- payload 仍只传 ticket，不传医疗明细。
- 离开页面立即停止扫描和广播。

新增成员页第二阶段增加“接收附近分享”入口：

- 入口放在新增成员页右上角，和扫码入口同级，避免放到首页。
- 可使用菜单承载两个动作：扫一扫、接收附近分享。
- 点击“接收附近分享”后进入接收状态，开始广播可接收服务。
- 收到 payload 后解析 ticket，成功后切换为绑定成员模式。
- 取消接收时停止广播并回到新增成员模式。

验收要求：

- 两台 iPhone 前台 5 秒内可互相发现。
- 拒绝蓝牙权限有明确错误态。
- 发送失败可重试。
- 接收成功后进入新增成员页绑定成员模式。
- 锁屏、切后台、退出页面后停止广播。
...
- 同一成员对同一用户重复邀请合并。
- 手机号/邮箱邀请需要脱敏展示。
- 被邀请人接受前只展示成员摘要，不展示医疗明细。
- 邀请过期时间，例如 7 天。
- 邀请人失去分享权限后，未接受邀请自动失效。
- 成员被删除后，邀请自动失效。
- 操作记录写审计日志。
...
```text
POST /api/v1/medical/members/{member_id}/invites/
GET  /api/v1/medical/member-invites/pending/
POST /api/v1/medical/member-invites/{invite_id}/accept/
POST /api/v1/medical/member-invites/{invite_id}/reject/
POST /api/v1/medical/member-invites/{invite_id}/cancel/
PATCH /api/v1/medical/member-bindings/{binding_id}/role/
DELETE /api/v1/medical/member-bindings/{binding_id}/
POST /api/v1/medical/member-bindings/{binding_id}/transfer-owner/
```

响应要求：

- 所有错误有明确 `msg`，例如 `invite_expired`、`already_bound`、`target_user_not_found`、`permission_denied`。
- 403 只表示无权限，不能让客户端退出登录。
- 接受邀请成功后返回成员列表项或成员详情摘要，方便客户端刷新。

### 16.10 客户端页面建议

新增或增强页面：

- `PendingMemberInvitesView`：待处理成员邀请列表。
- `MemberInviteAcceptPanel`：新增成员页绑定模式的数据来源之一。
- `MemberSharedUsersManageView`：共享用户管理。
- `MemberBindingRoleEditor`：角色编辑。
- `RemoteInviteFormView`：手机号/邮箱邀请表单。
- `NearbyShareReceivePanel`：新增成员页内的附近分享接收状态面板。
...
1. 用户 A 可通过手机号/邮箱/App 内通知邀请用户 B 绑定成员。
2. 用户 B 可在通知中心或待处理邀请列表看到邀请。
3. 用户 B 接受邀请时必须填写与成员的关系。
4. 用户 B 接受后生成永久 `UserMemberBinding`。
5. 用户 A 可在成员详情看到用户 B 的绑定记录。
6. owner 可移除共享用户。
7. 被移除用户不能再访问该成员医疗资料。
8. 邀请过期、取消、已绑定、无权限都有明确错误态。
9. 权限能力位由服务端返回，客户端不自行放权。
10. 附近设备分享在真机上具备可重试、可诊断和隐私保护。
11. `Member.relationship` 不再出现在服务端模型、接口字段和客户端 Member 主模型中，客户端不再把关系作为 Member 主字段维护。
12. 删除/解绑、分享 Provider、权限服务入口完成收敛，不再重复散落实现。
...
本章节作为远程邀请的补充需求，重点定义“点击发送邀请”后的异步投递、降级策略、用户反馈、通知点击路由和项目架构落点。该能力属于第二阶段。

### 17.1 目标

远程邀请不是只创建一条邀请记录，还需要尽可能通知到被邀请用户。发送邀请后，服务端按固定顺序尝试通知：
...
- 前一个通道成功后立即停止，不再执行后续通知。
- 前一个通道失败后才执行下一个通道。
- 三个通道都失败时，邀请记录仍可创建成功，但要明确告知分享人“通知用户失败，需要主动联系用户进入应用”。
- 通知点击后必须直接打开成员邀请页面，而不是只打开首页。

### 17.2 点击发送邀请后的异步状态

客户端点击“发送邀请”后，不应长时间卡死在按钮上。建议使用异步等待状态：

```text
idle
submittingInvite
waitingDeliveryResult
success(delivery_channel)
partialSuccess(invite_created_but_notify_failed)
failed(error)
```

UI 行为：

- 点击发送后按钮进入 loading，文案为“正在发送邀请...”。
- 服务端创建邀请并执行通知投递，客户端等待最终投递结果。
- 若接口耗时较长，客户端保持当前 Sheet，不自动关闭。
- 成功后展示结果文案，并关闭或回到共享用户页。
- 失败时保留输入内容，允许重试。
...
- 如果目标用户存在可用 APNs token，优先发送 APNs。
- 如果目标用户当前在线或服务端能识别 App 内会话，可走已有应用内通知流程。
- APNs 或 App 内通知任一成功，即返回：

```json
{
  "msg": "invite_sent_by_app_notification",
  "delivery_channel": "app_notification",
  "display_message": "邀请成功，已通过应用通知用户"
}
```

- 成功后不再发送邮箱和短信。
- 如果 APNs token 不存在、通知关闭、推送服务失败、设备失效或应用内通知失败，进入邮箱投递。

第二优先级：邮箱

- 目标用户有邮箱，或邀请表单填写了邮箱时尝试。
- 邮件发送成功，即返回：

```json
{
  "msg": "invite_sent_by_email",
  "delivery_channel": "email",
  "display_message": "邀请成功，已邮件通知用户"
}
```

- 成功后不再发送短信。
- 邮箱为空、邮箱格式无效、邮件服务失败、退信同步失败等情况，进入短信投递。

第三优先级：短信

- 目标用户有手机号，或邀请表单填写了手机号时尝试。
- 短信发送成功，即返回：

```json
{
  "msg": "invite_sent_by_sms",
  "delivery_channel": "sms",
  "display_message": "邀请成功，已短信通知用户"
}
```

- 如果短信也失败，邀请记录仍可保留为 pending，返回：

```json
{
  "msg": "invite_created_notify_failed",
  "delivery_channel": "none",
  "display_message": "分享成功，但通知用户失败，请主动联系用户进入应用查看邀请"
}
```
...
```json
{
  ...
  "display_message": "邀请成功，已通过应用通知用户",
  "open_url": "spark://member-invite?id=1001"
}
```
...
- `invite_id`：远程邀请记录 ID。
- `status`：邀请状态，创建后通常为 `pending`。
- `delivery_channel`：`app_notification` / `email` / `sms` / `none`。
- `delivery_status`：`sent` / `failed` / `skipped`。
- `display_message`：客户端可直接展示的结果文案。
- `open_url`：通知、邮件、短信统一使用的应用端链接。

错误规则：

- 成员不存在：`member_not_found`。
- 无分享权限：`permission_denied`。
- 目标用户已绑定：`already_bound`。
- 目标账号不存在但允许通过邮箱/手机号邀请：创建 pending 邀请并走邮箱/短信。
- 目标账号不存在且没有可用联系方式：`target_user_not_found`。
- 邀请创建失败：返回失败，不执行通知。
- 邀请创建成功但通知失败：返回业务成功，`delivery_channel = none`，`delivery_status = failed`。
...
```text
spark://member-invite?id={invite_id}
```

可选补充参数：

```text
spark://member-invite?id={invite_id}&source=apns
spark://member-invite?id={invite_id}&source=email
spark://member-invite?id={invite_id}&source=sms
```
...
- 通知、邮件、短信只展示成员基础摘要，不展示医疗资料。
- 如果成员名属于敏感信息，短信可降级为“你收到一条成员绑定邀请”。
- 链接只能打开邀请确认页，不能直接完成绑定。
- 接受绑定前仍必须填写 relationship。
...
- 用户点击系统通知。
- App 启动或恢复。
- `PushAdapter.didReceive` 解析 payload。
- `HandleRemoteNotificationUseCase` 交给 `RouteCoordinator`。
- `RouteCoordinator` 路由到成员邀请页面。

应用运行在前台：

- `PushAdapter.willPresent` 不展示系统 banner。
- 走项目已有公共通知流程 `NotificationClient`。
- 应用内通知展示为可点击 banner。
- 用户点击应用内通知后直接打开成员邀请页面。
- 前台不应同时展示系统通知和应用内通知，避免重复。
...
- 正文：邀请人、成员摘要、操作说明。
- 主按钮：打开 Spark 查看邀请。
- 链接：`spark://member-invite?id={invite_id}&source=email`。

客户端要求：

- 从邮件点击链接后，App 打开成员邀请页面。
- 未登录时先进入登录；登录完成后继续打开成员邀请页面。
- 如果邀请已过期，展示邀请失效页。
- 如果已绑定，展示已绑定状态并提供进入成员详情。

### 17.8 短信通知行为

短信发送成功后，服务端返回“邀请成功，已短信通知用户”。

短信内容需要短：

```text
你收到一条 Spark 成员绑定邀请，点击打开：spark://member-invite?id=1001&source=sms
```

如果系统或运营商不支持直接识别自定义 scheme，可使用短链承载：

```text
https://spark.example.com/i/member-invite/1001
```

短链打开规则：

- 已安装 App：跳转 App 并进入成员邀请页面。
- 未安装 App：展示安装引导页。
- 不在第二阶段做 Web 端接受绑定。
...
- 不再继续其他通知通道。
- 如果邀请记录已创建，返回“分享成功，但通知用户失败，请主动联系用户进入应用查看邀请”。
- 客户端可展示“复制邀请说明”或“稍后重试通知”。

### 17.9 客户端架构落点

现有项目已有通知与路由基础：

- `PushAdapter`：接收 APNs、前台通知、通知点击。
- `HandleRemoteNotificationUseCase`：处理远程通知 payload。
- `NotificationClient`：发布应用内 toast/banner/alert。
- `NotificationHostView`：展示应用内通知。
- `RouteCoordinator`：统一消费通知和深链。
- `AppRouteStore` / `AppRoute`：typed route graph。
...
- APNs payload：`route = member_invite` + `invite_id`。
- 应用端链接：`spark://member-invite?id=1001`。
- 邮箱/短信短链最终也转换为同一个 AppRoute。
...
Projects/Features/MemberContext/
  Application/
    MemberInviteUseCase.swift
  Infrastructure/
    MemberInviteRepository.swift
  Presentation/
    PendingMemberInvitesView.swift
    MemberInviteAcceptView.swift
    RemoteInviteFormView.swift
```

说明：

- `MemberInviteAcceptView` 可以复用新增成员页的“绑定成员模式”组件，但远程邀请详情来自 `invite_id`，不是二维码 ticket。
- 如果用户未登录，先缓存 `invite_id`，登录成功后继续路由。
- App 内通知 banner 需要支持点击动作，点击后调用 `routeStore.route(to: .memberInvite(inviteID), replaceStack: true)`。
...
```text
medical/services/member_invite_service.py
medical/services/member_invite_delivery.py
```

职责划分：

- `member_invite_service`：创建邀请、校验权限、接受/拒绝/取消邀请。
- `member_invite_delivery`：按 APNs -> 邮箱 -> 短信顺序投递通知。
- 通知服务适配层：复用项目已有 APNs/device 通知流程。
- 邮件服务适配层：封装邮件模板和发送结果。
- 短信服务适配层：封装短信模板和发送结果。
...
```python
def create_invite_and_notify(inviter, member, target):
    invite = create_invite(inviter=inviter, member=member, target=target)

    if send_app_notification(invite):
        return invite, DeliveryResult("app_notification", "sent", "邀请成功，已通过应用通知用户")

    if send_email(invite):
        return invite, DeliveryResult("email", "sent", "邀请成功，已邮件通知用户")

    if send_sms(invite):
        return invite, DeliveryResult("sms", "sent", "邀请成功，已短信通知用户")

    return invite, DeliveryResult("none", "failed", "分享成功，但通知用户失败，请主动联系用户进入应用查看邀请")
```
...
```text
home.members.invite.remote.title
home.members.invite.remote.send
home.members.invite.remote.sending
home.members.invite.remote.sent_app
home.members.invite.remote.sent_email
home.members.invite.remote.sent_sms
home.members.invite.remote.notify_failed
home.members.invite.remote.open
home.members.invite.remote.accept
home.members.invite.remote.reject
home.members.invite.remote.expired
home.members.invite.remote.already_bound
home.members.invite.remote.relationship_required
```

### 17.13 验收标准

1. 点击发送邀请后，客户端进入异步等待状态。
2. APNs/App 内通知成功时，接口返回“邀请成功，已通过应用通知用户”，不再发送邮箱/短信。
3. APNs/App 内通知失败且邮箱成功时，接口返回“邀请成功，已邮件通知用户”，不再发送短信。
4. APNs 和邮箱失败且短信成功时，接口返回“邀请成功，已短信通知用户”。
5. 三个通知通道都失败时，邀请仍创建成功，客户端提示“分享成功，但通知用户失败，请主动联系用户进入应用查看邀请”。
6. App 未启动时点击 APNs，打开 App 后直接进入成员邀请页面。
7. App 前台收到邀请通知时，只展示应用内通知，不展示系统 banner。
8. 应用内通知可点击，点击后进入成员邀请页面。
9. 邮箱链接可打开 App 并进入成员邀请页面。
10. 短信链接可打开 App 并进入成员邀请页面。
11. 未登录用户点击链接后，登录完成继续进入成员邀请页面。
12. 接受邀请前必须填写 relationship。
13. 接受成功后创建永久 `UserMemberBinding` 并刷新成员列表。

---

## 18. 成员分享权限分级与统一服务端控制

本章节补充成员分享后的权限控制需求。权限控制必须走统一公共管理逻辑，不能在各个接口、各个页面、各个 workflow 中分散手写判断。当前阶段只要求服务端做权限控制；客户端当前阶段不做强控制，若客户端请求超出权限限制，服务端返回操作未成功即可。

### 18.1 权限分级

所有者分享成员给其他用户时，可以设置被分享用户的权限。第一版权限分为三档：
...
- “删除”指删除/解绑共享成员相关资源的高危操作，具体删除成员档案仍需遵守成员详情里的删除语义。
- “编辑”指修改已有成员资料或成员下已有医疗资料。
- “新增”指上传报告、创建病历、创建服药计划、保存 AI 识别结果等新增医疗资料。
- “查看”指读取成员资料、医疗资料、附件和共享状态。
...
如果继续复用 `UserMemberBinding.role` 字段，也应明确映射关系：
...
不建议让客户端自行根据 role 推导权限；服务端仍需返回能力位，并在接口层强制校验。

### 18.2 分享时设置权限

分享页面需要支持设置权限：

- 二维码分享：生成 ticket 前选择权限。
- 附近设备分享：生成 nearby ticket 前选择权限。
- 远程邀请：发送邀请前选择权限。

分享页默认权限建议：

- 默认选择“编辑权限”，适合家庭成员共同维护资料。
- 如果产品希望更保守，可默认“查看权限”。
- “管理权限”需要二次确认，提示对方将具备删除能力。
...
- 权限选择必须在生成 `share_ticket` 或远程邀请前完成。
- `share_ticket` / `MemberShareInvite` 必须记录本次分享授予的权限。
- 被邀请用户接受后，创建 `UserMemberBinding` 时写入对应权限。
- 已生成的二维码/附近 ticket 不应在客户端本地改权限；改权限必须重新生成 ticket。
...
- 所有者可以查看所有绑定用户权限。
- 所有者可以调整其他用户权限。
- 管理权限用户是否可以调整其他用户权限，后续可配置；当前建议只有 owner 可调整。
- 用户不能把自己的 owner 权限降级到无 owner 状态，除非先转让 owner。
...
3. 选择某个绑定用户。
4. 选择新的权限档位。
5. 服务端更新 `UserMemberBinding.permission_level` 或等价字段。
6. 返回更新后的共享用户列表和能力位。

接口建议：

```text
PATCH /api/v1/medical/member-bindings/{binding_id}/permission/
```
...
```text
medical/services/member_permission_service.py
```

统一服务至少提供：

```text
ensure_can_view_member(user, member_id)
ensure_can_create_member_resource(user, member_id)
ensure_can_edit_member_resource(user, member_id)
ensure_can_delete_member_resource(user, member_id)
ensure_can_manage_member_bindings(user, member_id)
```
...
- 在 ViewSet 中各自写 `member.user == request.user`。
- 在 serializer 中各自复制权限判断。
- 在 workflow 中手写临时 if。
- 让客户端 role 判断替代服务端授权。
...
- “解绑自己”永远允许，避免用户无法退出共享关系。
- “再次分享成员”当前建议只有 manage 允许；若产品希望 edit 也能分享，需要服务端能力位控制。
- 删除成员档案仍需额外校验 owner、绑定人数和二次确认，不能只看 manage。
...
- 可以展示服务端返回的权限档位和能力位。
- 可以在 UI 上做基础置灰，但不能依赖客户端置灰作为安全边界。
- 客户端发起超权限请求时，服务端返回 403 或业务错误。
- 客户端收到 403 只展示操作失败，不退出登录。
...
远程邀请页面中的邮箱输入框需要符合 iOS 输入体验：

- 使用邮件键盘：`keyboardType(.emailAddress)`。
- 关闭首字母自动大写：`textInputAutocapitalization(.never)`。
- 可关闭自动纠错：`autocorrectionDisabled(true)`。
- 键盘上方需要“完成”按钮，用于收起键盘。
...
- 该要求对应远程邀请表单里的邮箱输入框。
- 分享页权限选择与远程邀请邮箱输入属于同一条邀请链路，但职责不同：权限选择决定绑定后的能力，邮箱输入决定投递目标。

### 18.8 远程邀请手机号区号与匹配规则

远程邀请支持手机号时，必须处理“系统登记手机号带区号，但用户输入不带区号”导致匹配不到的问题。
...
- 账号体系中的手机号建议按 E.164 格式存储，例如 `+8615385056020`。
- 用户输入时经常只输入本地手机号，例如 `15385056020`。
- 如果服务端直接按字符串精确匹配，会导致 `15385056020` 无法匹配 `+8615385056020`。

存储规范：

- 服务端账号手机号统一存储为 E.164 格式。
- 中国大陆手机号统一存为 `+86` + 11 位手机号。
- 不建议存储多个非规范手机号副本作为主匹配字段。
- 可额外保存 `phone_country_code`、`phone_national_number`、`phone_e164`，方便查询和展示。

客户端输入建议：

- 手机号输入框默认国家/地区为中国大陆 `+86`。
- UI 上展示区号选择器或默认区号标签。
- 用户输入 `15385056020` 时，客户端可在提交前规范化为 `+8615385056020`。
- 用户输入已经带 `+86` 时，不重复添加区号。
- 用户输入包含空格、短横线、括号时，提交前去除格式字符。

公共组件要求：

- 复用项目中手机号登录的录入组件能力，不再为远程邀请单独写一套手机号输入。
- 将手机号登录输入能力抽成公共组件，例如 `PhoneNumberInputView` / `PhoneNumberInputField`。
- 公共组件同时服务于手机号登录、远程邀请、后续账号绑定手机号等场景。
- 组件输出必须包含：
  - `rawInput`：用户原始输入。
  - `countryCode`：当前选择区号，例如 `+86`。
  - `nationalNumber`：去格式后的本地号码，例如 `15385056020`。
  - `e164`：规范化结果，例如 `+8615385056020`。
  - `isValid`：基础格式是否合法。
- 组件内部负责去除空格、短横线、括号等格式字符。
- 组件内部负责避免 `+86` 重复拼接。
- 远程邀请提交接口优先传 `e164`，同时可传 `country_code` 和 `national_number` 方便服务端兜底。
...
- 默认区号：中国大陆 `+86`。
- 区号可以后续扩展为国家/地区选择器。
- 当前阶段至少展示默认区号，避免用户不知道系统会补 `+86`。
- 键盘建议使用电话键盘：`keyboardType(.phonePad)`。
- phonePad 没有系统 return 键时，键盘上方需要提供“完成”按钮收起键盘。

服务端兜底规则：

1. 接收用户输入 `raw_phone`。
2. 去除空格、短横线、括号等格式字符。
3. 如果以 `+` 开头，按 E.164 解析并匹配。
4. 如果不带 `+`，使用请求中的 `country_code`；缺失时默认使用当前用户地区或产品默认地区 `+86`。
5. 将本地号码转换为 E.164 后查询用户。
6. 若 E.164 查询失败，可再用 `national_number + country_code` 组合字段查询。
7. 若仍失败，按“目标用户不存在但可短信邀请”处理，走短信通道。

匹配示例：

| 输入 | country_code | 规范化结果 | 匹配目标 |
| --- | --- | --- | --- |
| `15385056020` | `+86` | `+8615385056020` | 可匹配系统登记 `+8615385056020` |
| `+8615385056020` | 任意 | `+8615385056020` | 可直接匹配 |
| `86 15385056020` | 空 | `+8615385056020` | 识别为中国区号后匹配 |
| `153-8505-6020` | `+86` | `+8615385056020` | 去格式后匹配 |
...
```json
{
  "normalized_phone": "+8615385056020",
  "matched_user_id": 123
}
```
...
- 手机号格式错误：`phone_invalid`。
- 国家区号缺失且无法推断：`country_code_required`。
- 未匹配到 App 用户但手机号有效：创建邀请并走短信通知。
- 短信也失败：提示“分享成功，但通知用户失败，请主动联系用户进入应用查看邀请”。

实现要求：

- 手机号规范化逻辑必须在服务端集中实现，不能由多个接口各自拼接 `+86`。
- 客户端可以做预规范化提升体验，但服务端必须兜底。
- 匹配账号时必须使用规范化后的手机号，避免同一个手机号因格式不同匹配失败。
- 日志中展示手机号需脱敏，例如 `+86153****6020`。

### 18.9 工单 `MEMBER-INVITE-000001`：远程邀请联系人匹配按 bundle_id 隔离

#### 18.9.1 问题背景

成员远程邀请新增时，服务端会根据邮箱或手机号查找目标用户，并在邀请记录中写入 `target_user`。当前发现同一手机号在不同 App bundle 下绑定了不同用户时，邀请接口可能匹配到错误 App 的用户。邮箱邀请也存在同类问题：`auth_user.email` 不是 App 维度唯一，同一个邮箱可能出现在多个用户上，必须结合 `accounts_socialidentity.bundle_id` 判断当前 App 下应该发给哪个用户。

示例数据：

| id | provider | provider_uid | bundle_id | user_id |
| --- | --- | --- | --- | --- |
| 24 | phone | `+8615385056020` | `cn.Zhaodk.Aera` | 31 |
| 632 | phone | `+8615385056020` | `cn.Zhaodk.Health` | 695 |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `Zhaodk` | `97621528@qq.com` | 1 | 1 | `2025-11-27 04:20:57.562640` |
| 265 | `apple_000082` | `97621528@qq.com` | 0 | 1 | `2026-03-29 21:13:20.562955` |

`accounts_socialidentity`：

| id | provider | provider_uid | bundle_id | user_id |
| --- | --- | --- | --- | --- |
| 205 | apple | `000082.58f2a98183e84bfbb26777c41c443af9.1112` | `cn.Zhaodk.Health` | 265 |

当 Health App 通过邮箱 `97621528@qq.com` 邀请成员时，服务端不能只执行 `User.objects.filter(email__iexact=email).first()`，否则可能命中全局管理员用户 `id = 1`。必须先用 `auth_user.email = 97621528@qq.com` 找到候选用户，再通过 `accounts_socialidentity.user_id = auth_user.id` 且 `accounts_socialidentity.bundle_id = cn.Zhaodk.Health` 确认当前 App 下的目标用户，最终应匹配 `user_id = 265`。

请求来自 `cn.Zhaodk.Health`：

```text
POST /api/v1/medical/members/494/invites/
Authorization token.bundle_id = cn.Zhaodk.Health
body.phone = 15385056020
body.country_code = +86
```
...
- 同一联系方式跨 bundle 存在多条 `SocialIdentity` 时，接口可能返回 `matched_user_id = 31`。
- 同一邮箱存在多条 `auth_user.email` 时，接口可能只按邮箱命中错误用户，例如把 Health 邀请发给全局管理员 `user_id = 1`，而不是 Health 身份用户 `user_id = 265`。
- 邀请会投递给 `cn.Zhaodk.Aera` 下的账号，导致 Health App 内目标用户收不到正确邀请。
- 已存在 pending 邀请时，重复请求还可能复用错误 `target_user` 的旧邀请记录，使后续响应出现权限、permission 或 invite_id 不一致。

#### 18.9.2 根因判断

远程邀请链路的联系人解析没有把当前请求的 `bundle_id` 带入账号身份查询。
...
邮箱查询也必须遵守同一规则。当前邮箱主要在 `auth_user.email` 内，因此不能只按 `auth_user.email` 取第一条用户。必须将 `auth_user.email` 命中的候选用户与 `accounts_socialidentity.bundle_id` 联合确认：只有候选用户存在当前 `bundle_id` 下的 `accounts_socialidentity` 记录，才可作为邀请目标。若后续新增 `SocialIdentity(provider=email)`，也必须查询 `(bundle_id, provider=email, provider_uid=normalized_email)`。

#### 18.9.3 修复方案

服务端以当前认证上下文中的 bundle 为唯一匹配维度：

1. 从 access token claim、设备会话或请求认证上下文中读取 `bundle_id`，例如 `cn.Zhaodk.Health`。
2. `MemberShareInviteCreateView` 调用 `resolve_user_by_contact` 时显式传入 `bundle_id`。
3. `resolve_user_by_contact(channel, contact, country_code, bundle_id)` 对手机号、邮箱都按 bundle 过滤。
4. 手机号先按第 18.8 节规范化为 E.164，再查询 `(bundle_id, provider=phone, provider_uid=e164)`。
5. 邮箱先 trim/lowercase；查询时先按 `auth_user.email__iexact = normalized_email` 找候选用户，再通过 `accounts_socialidentity.user_id = auth_user.id` 且 `accounts_socialidentity.bundle_id = current_bundle_id` 确认目标用户。若多个候选用户使用同一邮箱，只允许返回当前 bundle 下存在身份记录的用户。
6. 创建或复用 pending 邀请时，去重条件应包含 `member_id`、`inviter_user_id`、`target_user_id`、`channel`、`role` 和规范化后的 `target_contact`，避免错误目标用户的历史 pending 邀请被复用。
7. 响应中的 `matched_user_id` 必须来自当前 bundle 的身份匹配结果；查不到当前 bundle 用户时返回 `null`，再走邮箱或短信直投逻辑。

建议代码落点：

| 文件 | 修改点 |
| --- | --- |
| `SparkService/medical/views.py` | `MemberShareInviteCreateView.post` 读取当前请求 `bundle_id`，传给联系人解析服务 |
| `SparkService/medical/services/member_invite_service.py` | `resolve_user_by_contact` 增加 `bundle_id` 参数，手机号查询按 `(bundle_id, provider, provider_uid)` 过滤；邮箱查询按 `auth_user.email + accounts_socialidentity.bundle_id` 确认 |
| `SparkService/medical/services/member_invite_delivery.py` | 复用 pending 邀请和投递目标时使用已规范化且 bundle 隔离后的 `target_user` |
| `SparkService/medical/tests_member_permission.py` 或新增邀请测试文件 | 增加同手机号跨 bundle、同邮箱跨 bundle 的回归测试 |

#### 18.9.4 兼容与安全要求

- `bundle_id` 缺失时，不允许退回到全局手机号匹配；应返回明确错误或按“未匹配 App 用户但联系方式有效”处理，不能误绑其他 App 用户。
- `SocialIdentity` 已有唯一约束 `(bundle_id, provider, provider_uid)`，远程邀请查询必须使用同一业务键。
- 邮箱邀请不得依赖 `auth_user.email` 的全局唯一性；即使数据库中存在多个相同邮箱，也必须用当前 bundle 下的 `accounts_socialidentity` 记录确认最终用户。
- 日志中可以记录 `bundle_id`、`channel`、`matched_user_id` 和 request_id，但手机号、邮箱必须脱敏。
- 修复后不影响短信或邮件直投：当前 bundle 未匹配到 App 用户时，仍可以创建 `target_user = null` 的 pending 邀请，并按填写的联系方式投递。
- 不允许客户端传入的 body `bundle_id` 覆盖认证上下文中的 bundle；客户端传入值仅可用于诊断，最终以 token / session bundle 为准。

#### 18.9.5 回归测试

必须覆盖：

1. 同一手机号 `+8615385056020` 在 `cn.Zhaodk.Aera` 和 `cn.Zhaodk.Health` 下分别属于用户 31 和 695；Health token 发起邀请时，`matched_user_id = 695`。
2. Aera token 发起同样手机号邀请时，`matched_user_id = 31`。
3. 当前 bundle 下没有对应手机号身份，但其他 bundle 有该手机号时，不返回其他 bundle 用户，`matched_user_id = null`，邀请可继续走短信直投。
4. 同一邮箱 `97621528@qq.com` 同时存在 `auth_user.id = 1` 和 `auth_user.id = 265`，且只有用户 265 存在 `accounts_socialidentity(bundle_id="cn.Zhaodk.Health")` 时，Health token 发起邮箱邀请必须返回 `matched_user_id = 265`。
5. 同一邮箱跨 bundle 绑定不同用户时，邮箱邀请只匹配当前 bundle 用户。
6. 当前 bundle 下没有对应邮箱身份，但其他 bundle 有该邮箱时，不返回其他 bundle 用户，邀请可继续走邮件直投。
7. 重复发送邀请时，不复用其他 `target_user_id`、其他 `role` 或其他规范化联系方式的 pending 邀请。
8. 响应日志和接口响应中的 `normalized_phone` 正确，敏感日志脱敏。

#### 18.9.6 验收标准

1. Health App 发起手机号邀请 `15385056020` 时，只匹配 `SocialIdentity(bundle_id="cn.Zhaodk.Health", provider="phone", provider_uid="+8615385056020")`。
2. Aera App 发起同一手机号邀请时，只匹配 `cn.Zhaodk.Aera` 下的身份。
3. 邮箱邀请也按当前认证 bundle 隔离，不跨 App 匹配全局用户；匹配时必须通过 `auth_user.email` 命中候选用户，并用 `accounts_socialidentity.bundle_id` 确认当前 App 下的最终用户。
4. 修复后接口响应 `matched_user_id` 与当前 token 的 `bundle_id` 一致。
5. 当前 bundle 未命中账号时，不误用其他 bundle 用户，仍允许按邮箱/短信直投创建 pending 邀请。
6. 自动化测试覆盖同手机号、同邮箱跨 bundle 的正反向场景。

### 18.10 APNs / 应用内通知触发成员邀请页

成员远程邀请通知需要和项目现有通知、路由架构打通。已有日志示例：

```text
路由流程：消费远程通知 route=memberInvite(inviteID: 17)
```

这类通知不能只路由到首页，也不能只刷新普通任务同步；必须触发成员邀请相关状态刷新和页面展示。

前台运行时：

- App 正在运行且收到 APNs 成员邀请通知时，`PushAdapter.willPresent` 不展示系统 banner。
- 通知进入项目已有公共通知流程，展示应用内顶部弹窗。
- 收到通知后默认执行 `memberInviteUseCase.fetchPending`，刷新待处理邀请数量。
- 如果用户不点击顶部弹窗，只刷新数量，不强制打断当前页面。
- 如果用户点击顶部弹窗，直接以 sheet 打开 `PendingMemberInvitesView`。

应用外 / 未启动时：

- 用户点击系统 APNs 通知。
- App 启动或回到前台。
- `PushAdapter.didReceive` 解析 payload，得到 `route = memberInvite(inviteID: 17)`。
- `RouteCoordinator` 消费远程通知 route。
- 进入应用后以 sheet 打开 `PendingMemberInvitesView`。
- 页面打开后调用 `memberInviteUseCase.fetchPending`，并可定位或高亮 `inviteID = 17`。

页面承载方式：

```text
HomeView / MainTabCoordinatorView
  .sheet(isPresented: showPendingMemberInvites) {
...
```swift
@Published var pendingMemberInviteRoute: PendingMemberInviteRoute?
@Published var pendingMemberInviteCount: Int = 0
```

处理规则：

1. 收到成员邀请通知，解析 `invite_id`。
2. 调用 `memberInviteUseCase.fetchPending`。
3. 更新 pending 邀请数量。
4. 如果 entryPoint 是 `.foreground`，只展示可点击应用内通知。
5. 如果用户点击应用内通知，打开 `PendingMemberInvitesView` sheet。
6. 如果 entryPoint 是 `.interaction`，直接打开 `PendingMemberInvitesView` sheet。
7. sheet 打开后再次刷新 pending 列表，避免通知到达和用户点击之间状态变化。

应用内通知点击能力：

- 当前 `NotificationMessage` 需要支持可选 action。
- 成员邀请通知的 action 为 `open_member_invites`。
- `NotificationBannerView` 除关闭按钮外，整条 banner 或主区域可点击。
- 点击后关闭 banner，并通知 `RouteCoordinator` / `AppRouteStore` 打开成员邀请 sheet。
...
- 前台收到邀请通知后同时展示系统 banner 和应用内 banner。
- 用户点击通知后只进入首页但不打开邀请页。
- 只触发任务增量同步，不刷新成员邀请数量。
- 在通知点击时直接接受邀请；必须进入邀请页面，由用户确认关系后接受。

### 18.11 验收标准

1. 分享页面可以选择三档权限：管理权限、编辑权限、查看权限。
2. 远程邀请可以携带所选权限。
3. 二维码/附近设备 ticket 可以携带所选权限。
4. 被邀请用户接受后，`UserMemberBinding` 保存对应权限。
5. 共享用户管理可以调整绑定用户权限。
6. 服务端所有成员医疗接口按统一权限服务判断。
7. 查看权限用户发起新增、编辑、删除请求时，服务端拒绝。
8. 编辑权限用户可以新增和编辑，但不能删除。
9. 管理权限用户可以删除、编辑、新增、查看。
10. 客户端收到超权限错误只提示操作失败，不退出登录。
11. 远程邀请邮箱输入框使用邮件键盘。
12. 邮箱键盘上方有“完成”按钮可收起键盘。
13. 手机号 `15385056020` 在默认 `+86` 下可匹配系统登记的 `+8615385056020`。
14. 用户输入已带 `+86` 时，不会重复添加区号。
15. 服务端手机号匹配逻辑使用统一规范化服务，不在各接口分散实现。
16. 远程邀请手机号输入复用手机号登录公共组件。
17. 手机号公共组件输出 `countryCode`、`nationalNumber`、`e164` 和 `isValid`。
18. App 前台收到成员邀请 APNs 时，默认刷新 `memberInviteUseCase.fetchPending` 邀请数量。
19. 用户点击应用内顶部邀请通知时，以 sheet 打开 `PendingMemberInvitesView`。
20. 用户在应用外点击 APNs 通知进入 App 后，以 sheet 打开 `PendingMemberInvitesView`。
21. `PendingMemberInvitesView` 打开后刷新 pending 邀请列表，并可定位通知对应的 `inviteID`。
22. 远程邀请通过手机号或邮箱匹配 App 用户时，必须按当前认证上下文的 `bundle_id` 隔离。
23. 同一手机号或邮箱存在多个 bundle 身份时，接口返回的 `matched_user_id` 必须属于当前请求 bundle。
