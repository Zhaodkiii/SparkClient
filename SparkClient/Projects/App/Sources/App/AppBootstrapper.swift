...
final class AppBootstrapper {
    ...
    func bootstrapAppLaunchIfNeeded() async {
        ...
        // 设备登记在会话恢复后由 `AppLifecycleCoordinator` 显式触发（未登录 appLaunch / 已登录 signedInColdLaunch / 登录 signedInBootstrap）。
        logger.info("应用启动引导（设备登记在会话恢复后由 AppLifecycleCoordinator 统一触发）", module: .general)
    }
