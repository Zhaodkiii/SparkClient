...
struct APIKeysSettingsView: View {
            ...
            if snapshot.trial.isActive, trialProviders.isEmpty == false {
                Section("试用期可用厂商") {
                    ForEach(trialProviders) { provider in
                        HStack(spacing: 12) {
                            Image(systemName: providerSymbol(for: provider.company))
    ...
    private var selectedProviderBinding: Binding<APIKeys?> {
        Binding<APIKeys?>(
            get: {
                guard let id = selectedProviderID else { return nil }
                return snapshot.apiKeys.first(where: { $0.id == id })
            },
            set: { newValue in
    ...
    private func setProviderEnabled(providerID: UUID, enabled: Bool) {
        ...
        if enabled && provider.key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            showError("\(provider.displayName) 需要先配置有效 API Key")
            return
        }
    ...
    private func providerSymbol(for company: String) -> String {
        switch company.uppercased() {
        case "OPENAI": return "brain.head.profile"
        case "GOOGLE": return "globe"
        case "ANTHROPIC": return "text.bubble"
        case "DEEPSEEK": return "sparkles"
        case "ZHIPUAI": return "hexagon"
        case "QWEN": return "bolt.horizontal"
