...
public struct SecondCameraEditorDataImageSource: SecondCameraEditorImageSource {
    ...
    public static func forPath(_ filePath: String) throws -> Self {
        do {
            ...
            SecondCameraEditorLogger.warn("Could not read image data: \(error)")
            throw error
        }
    }
...
extension SecondCameraEditorImageSource {
    ...
    private func ows_guessSecondCameraEditorImageFormat() -> SecondCameraEditorImageFormat? {
        ...
        case Data([0x52, 0x49]):
            ...
            return .webp
        default:
            return ows_guessHighEfficiencySecondCameraEditorImageFormat()
        }
    }
    ...
    public func imageMetadata() -> SecondCameraEditorImageMetadata? {
        ...
        assert(byteLimit >= SecondCameraEditorMediaUtils.kMaxFileSizeAnimatedImage)
        assert(byteLimit >= 4 * Int(SecondCameraEditorImageQualityTier.seven.maxEdgeSize) ^ 2 + 50_000)
        guard byteLength < byteLimit else {
            return nil
        }
        let imageFormat = ows_guessSecondCameraEditorImageFormat()
        guard let imageFormat else {
            SecondCameraEditorLogger.warn("Image does not have valid format.")
            return nil
        }
        guard let imageSource = try? self.cgImageSource() else {
            SecondCameraEditorLogger.warn("Could not build imageSource.")
            return nil
        }
...
private func isImageSizeValid(_ imageSize: CGSize, depthBytes: CGFloat) -> Bool {
    ...
    let maxValidImageDimension = SecondCameraEditorMediaUtils.kMaxImageDimensions
    let maxBytes = maxValidImageDimension * maxValidImageDimension * expectedBytesPerPixel

    if actualBytes > maxBytes {
        SecondCameraEditorLogger.warn("invalid dimensions width: \(imageSize.width), height \(imageSize.height), bytesPerPixel: \(bytesPerPixel)")
        return false
    }
...
private func imageMetadataWithImageSource(_ imageSource: CGImageSource, imageFormat: SecondCameraEditorImageFormat) -> SecondCameraEditorImageMetadata? {
    ...
    guard let widthNumber = imageProperties[kCGImagePropertyPixelWidth as String] as? NSNumber else {
        SecondCameraEditorLogger.warn("widthNumber was unexpectedly nil")
        return nil
    }
    guard let heightNumber = imageProperties[kCGImagePropertyPixelHeight as String] as? NSNumber else {
        SecondCameraEditorLogger.warn("heightNumber was unexpectedly nil")
        return nil
    }
    ...
    if let orientationNumber = imageProperties[kCGImagePropertyOrientation as String] as? NSNumber {
        guard let orientation = CGImagePropertyOrientation(rawValue: orientationNumber.uint32Value) else {
            SecondCameraEditorLogger.warn("orientation number was invalid")
            return nil
        }
    ...
    guard let depthNumber = imageProperties[kCGImagePropertyDepth as String] as? NSNumber else {
        SecondCameraEditorLogger.warn("depthNumber was unexpectedly nil")
        return nil
    }
    ...
    guard let colorModel = (imageProperties[kCGImagePropertyColorModel as String] as? NSString) as String? else {
        SecondCameraEditorLogger.warn("colorModel was unexpectedly nil")
        return nil
    }
    guard colorModel == kCGImagePropertyColorModelRGB as String || colorModel == kCGImagePropertyColorModelGray as String else {
        SecondCameraEditorLogger.warn("Invalid colorModel: \(colorModel)")
        return nil
    }

    guard isImageSizeValid(pixelSize, depthBytes: depthBytes) else {
        SecondCameraEditorLogger.warn("Image does not have valid dimensions: \(pixelSize).")
        return nil
    }
...
private struct SecondCameraEditorWebpMetadata {
    let canvasWidth: Int
