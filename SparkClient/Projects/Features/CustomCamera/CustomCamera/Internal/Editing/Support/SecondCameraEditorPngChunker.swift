...
///
///     let chunker = try SecondCameraEditorPngChunker(data: myPngData)
...
public class SecondCameraEditorPngChunker {
    ...
    private let pngSource: SecondCameraEditorImageSource
    ...
    /// - Throws: `SecondCameraEditorPngChunkerError.tooSmall` if the data is too small to possibly be a PNG.
    /// - Throws: `SecondCameraEditorPngChunkerError.fileDoesNotStartWithPngSignature` if the first 8 bytes aren't the PNG signature.
    public init(source: SecondCameraEditorImageSource) throws {
        guard source.byteLength >= Self.smallestPossiblePngSize else {
            throw SecondCameraEditorPngChunkerError.tooSmall
        }
        let prefix = try? source.readData(byteOffset: 0, byteLength: Self.pngSignature.count)
        guard prefix == Self.pngSignature else {
            throw SecondCameraEditorPngChunkerError.fileDoesNotStartWithPngSignature
        }
        pngSource = source
    ...
    /// - Throws: `SecondCameraEditorPngChunkerError.invalidChunkType` if a chunk's type is invalid.
    /// - Throws: `SecondCameraEditorPngChunkerError.invalidChunkChecksum` if a chunk's checksum is invalid.
    /// - Throws: `SecondCameraEditorPngChunkerError.endedUnexpectedly` if a chunk's length is longer than the remaining data available, or if the first chunk's length is too short.
    public func next() throws -> Chunk? {
        ...
        guard cursor + 8 <= pngSource.byteLength else {
            self.cursor = nil
            throw SecondCameraEditorPngChunkerError.endedUnexpectedly
        }

        let lengthBytes = try pngSource.readData(byteOffset: cursor, byteLength: 4)
        let length = try lengthBytes.secondCameraEditor_asPngUInt32()
        cursor += 4

        var expectedCrc = CRC32()

        let type = try pngSource.readData(byteOffset: cursor, byteLength: 4)
        guard type.secondCameraEditor_isValidPngType else {
            self.cursor = nil
            throw SecondCameraEditorPngChunkerError.invalidChunkType
        }
        ...
        let actualCrc = try crcBytes.secondCameraEditor_asPngUInt32()
        cursor += 4

        guard actualCrc == expectedCrc.value else {
            self.cursor = nil
            throw SecondCameraEditorPngChunkerError.invalidChunkChecksum
        }
    ...
    enum SecondCameraEditorPngChunkerError: Error {
...
extension Data {
    func secondCameraEditor_asPngUInt32() throws -> UInt32 {
        secondCameraEditorPrecondition(count == 4)
        var result: UInt32 = 0
    ...
    var secondCameraEditor_isValidPngType: Bool {
