...
public enum SecondCameraEditorMediaError: Error {
...
public enum SecondCameraEditorMediaUtils {
    ...
    public static func thumbnail(forImage image: UIImage, maxDimensionPixels: CGFloat) throws -> UIImage {
        ...
        guard let thumbnailImage = image.resized(maxDimensionPixels: maxDimensionPixels) else {
            throw SecondCameraEditorMediaError.failure(description: "Could not thumbnail image.")
        }
        guard nil != thumbnailImage.cgImage else {
            throw SecondCameraEditorMediaError.failure(description: "Missing cgImage.")
        }
    ...
    public static func validateVideoExtension(ofPath path: String) throws {
        ...
        guard SecondCameraEditorMimeTypeUtil.isSupportedVideoSecondCameraEditorMimeType(contentType) else {
            throw SecondCameraEditorGenericError("video file has invalid content type")
        }
    }
    ...
    public static func validateVideoAsset(_ asset: AVAsset) throws {
        ...
        if maxTrackSize.width < 1.0 || maxTrackSize.height < 1.0 {
            throw SecondCameraEditorGenericError("invalid video size: \(maxTrackSize)")
        }
        if maxTrackSize.width > kMaxVideoDimensions || maxTrackSize.height > kMaxVideoDimensions {
            throw SecondCameraEditorGenericError("invalid video dimensions: \(maxTrackSize)")
        }
    }
