...
nonisolated public enum SecondCameraEditorFormat {
    ...
    // formatting behavior.
    private nonisolated(unsafe) static let shortNameComponentsCache = SecondCameraEditorLRUCache<String, String>(
        maxSize: 512,
        nseMaxSize: 64,
...
public extension SecondCameraEditorFormat {
    ...
    static func formatNSInt(_ value: NSNumber) -> String {
        guard let value = defaultNumberFormatter.string(from: value) else {
            secondCameraEditorFailDebug("Could not format value.")
            return ""
        }
        return value
    }
...
public extension SecondCameraEditorFormat {
    ...
    static func localizedDecimalString(from number: Int) -> String {
        ...
        secondCameraEditorAssertDebug(result != nil, "Formatted string is nil. number=[\(number)]")
        return result ?? ""
    }
...
public extension SecondCameraEditorFormat {
    ...
    nonisolated static func localizedDurationString(from timeInterval: TimeInterval) -> String {
        ...
        secondCameraEditorAssertDebug(result != nil, "Formatted string is nil. ti=[\(timeInterval)]")
        return result ?? ""
    }
}
