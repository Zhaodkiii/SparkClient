...
class SecondCameraImageEditorCropViewController: SecondCameraEditorViewController {
    ...
    private let cropView = SecondCameraEditorCropView(frame: UIScreen.main.bounds)
    ...
    private lazy var resetButton: UIButton = {
        let button = SecondCameraRoundMediaButton(image: nil, backgroundStyle: .blur)
        let buttonTitle = SecondCameraEditorLocalizedString("MEDIA_EDITOR_RESET", comment: "Title for the button that resets photo to its initial state.")
        button.setTitle(buttonTitle, for: .normal)
        button.secondCameraEditor_contentEdgeInsets = UIEdgeInsets(hMargin: 26, vMargin: 15) // Make button 36pts tall at default text size.
    ...
    private lazy var footerView: UIView = {
        ...
        if UIDevice.current.secondCameraEditor_hasIPhoneXNotch {
    ...
    private lazy var rotationControl = SecondCameraRotationControl()
    private lazy var bottomBar: SecondCameraImageEditorBottomBar = {
        let bottomBar = SecondCameraImageEditorBottomBar(buttonProvider: self)
        bottomBar.cancelButton.addTarget(self, action: #selector(didTapSecondCameraCancel), for: .touchUpInside)
        bottomBar.doneButton.addTarget(self, action: #selector(didTapSecondCameraDone), for: .touchUpInside)
        return bottomBar
    }()

    init(model: SecondCameraImageEditorModel, srcImage: UIImage, previewImage: UIImage) {
        self.model = model
    ...
    override func viewDidLoad() {
        ...
        setupSecondCameraRotationControlActions()
        ...
        let topConstraint: NSLayoutConstraint = {
            if UIDevice.current.secondCameraEditor_hasIPhoneXNotch || UIDevice.current.secondCameraEditor_isIPad {
        ...
        let mediaTopBar = SecondCameraMediaTopBar()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        transitionSecondCameraEditorUI(toState: .final, animated: true)
    }
    ...
    private func applyTransformWithoutAnimation(_ transform: SecondCameraImageEditorTransform) {
    ...
    private func applyTransformWithAnimation(_ transform: SecondCameraImageEditorTransform, completion: ((Bool) -> Void)? = nil) {
        self.transform = transform
    ...
    private func applyTransformHidingCropFrame(_ transform: SecondCameraImageEditorTransform) {
    ...
    private func updateImageViewTransform() {
        ...
        let transformedFrame = SecondCameraImageEditorCanvasView.imageFrame(forViewSize: viewSize, imageSize: imageSize, transform: transform)
    ...
    private func transitionSecondCameraEditorUI(toState state: UIState, animated: Bool, completion: ((Bool) -> Void)? = nil) {
        ...
        let imageCornerRadius: CGFloat = state == .initial ? SecondCameraImageEditorView.defaultCornerRadius : 0
        if animated {
    ...
    private func configureGestureRecognizers() {
        let pinchGestureRecognizer = SecondCameraImageEditorPinchGestureRecognizer(target: self, action: #selector(handlePinchGesture(_:)))
        ...
        let panGestureRecognizer = SecondCameraImageEditorPanGestureRecognizer(target: self, action: #selector(handlePanGesture(_:)))
    ...
    private class func unitTranslation(
        ...
        viewBounds: CGRect,
        oldTransform: SecondCameraImageEditorTransform,
    ) -> CGPoint {
    ...
    private func handlePinchGesture(_ gestureRecognizer: SecondCameraImageEditorPinchGestureRecognizer) {
        ...
        case .changed, .ended:
            guard let gestureStartTransform else {
                secondCameraEditorFailDebug("Missing pinchTransform.")
                return
            }

            let unitTranslation =
                SecondCameraImageEditorCropViewController.unitTranslation(
            ...
            let clampedScaling = scaling.secondCameraEditor_clamp(SecondCameraImageEditorTextItem.kMinScaling, SecondCameraImageEditorTextItem.kMaxScaling)

            let newTransform = SecondCameraImageEditorTransform(
    ...
    private var gestureStartTransform: SecondCameraImageEditorTransform?
    private var panCropRegion: SecondCameraImageEditorCropRegion?
    private var isCropGestureActive: Bool {
        return panCropRegion != nil
    }

    @objc
    private func handlePanGesture(_ gestureRecognizer: SecondCameraImageEditorPanGestureRecognizer) {
        SecondCameraEditorAssertIsOnMainThread()
    ...
    private func handleCropPanGesture(
        _ gestureRecognizer: SecondCameraImageEditorPanGestureRecognizer,
        panCropRegion: SecondCameraImageEditorCropRegion,
    ) {
        SecondCameraEditorAssertIsOnMainThread()
        ...
        // Crop pan gesture
        let locationDelta = CGPoint.secondCameraEditor_subtract(locationNow, locationStart)
    ...
    private func crop(toRect cropRect: CGRect, animated: Bool) {
        ...
        let croppedOutputSizePixels = CGSize.secondCameraEditor_round(CGSize(
        ...
        // scaling accordingly.
        let naiveTransform = SecondCameraImageEditorTransform(
        ...
        let naiveImageFrameOld = SecondCameraImageEditorCanvasView.imageFrame(forViewSize: transform.outputSizePixels, imageSize: model.srcImageSizePixels, transform: naiveTransform)
        let naiveImageFrameNew = SecondCameraImageEditorCanvasView.imageFrame(forViewSize: croppedOutputSizePixels, imageSize: model.srcImageSizePixels, transform: naiveTransform)
        ...
        let oldImageFrameCanvas = SecondCameraImageEditorCanvasView.imageFrame(forViewSize: viewBounds.size, imageSize: model.srcImageSizePixels, transform: transform)
    ...
    private func handleNormalPanGesture(_ gestureRecognizer: SecondCameraImageEditorPanGestureRecognizer) {
        SecondCameraEditorAssertIsOnMainThread()
        ...
        let unitTranslation = SecondCameraImageEditorCropViewController.unitTranslation(
    ...
    private func cropRegion(forGestureRecognizer gestureRecognizer: SecondCameraImageEditorPanGestureRecognizer) -> SecondCameraImageEditorCropRegion? {
        guard let location = gestureRecognizer.locationFirst else {
            secondCameraEditorFailDebug("Missing locationStart.")
            return nil
        }

        let tolerance: CGFloat = SecondCameraEditorCropView.desiredCornerSize * 2.0
        let left = tolerance
        let top = tolerance
...
// MARK: - SecondCameraImageEditorBottomBarButtonProvider

extension SecondCameraImageEditorCropViewController: SecondCameraImageEditorBottomBarButtonProvider {

    var middleButtons: [UIButton] {
        let rotateButton = SecondCameraRoundMediaButton(
        ...
        let flipButton = SecondCameraRoundMediaButton(
        ...
        let aspectRatioButton = SecondCameraRoundMediaButton(
...
extension SecondCameraImageEditorCropViewController {
    ...
    enum AspectRatio: CaseIterable {
        ...
        private static func aspectRatioXByYFormatString() -> String {
            return SecondCameraEditorLocalizedString("ASPECT_RATIO_X_BY_Y", value: "%@:%@", comment: "Variable aspect ratio, eg 3:4. %1$@ and %2$@ are numbers.")
        }

        func localizedTitle() -> String {
            switch self {
            case .original:
                return SecondCameraEditorLocalizedString("ASPECT_RATIO_ORIGINAL", value: "原始", comment: "One of the choices for pre-defined aspect ratio of a photo in media editor.")
            case .square:
                return SecondCameraEditorLocalizedString("ASPECT_RATIO_SQUARE", value: "方形", comment: "One of the choices for pre-defined aspect ratio of a photo in media editor.")
            case .fourByThree:
                return String(format: AspectRatio.aspectRatioXByYFormatString(), SecondCameraEditorFormat.formatInt(4), SecondCameraEditorFormat.formatInt(3))
            case .threeByFour:
                return String(format: AspectRatio.aspectRatioXByYFormatString(), SecondCameraEditorFormat.formatInt(3), SecondCameraEditorFormat.formatInt(4))
            case .sixteenByNine:
                return String(format: AspectRatio.aspectRatioXByYFormatString(), SecondCameraEditorFormat.formatInt(16), SecondCameraEditorFormat.formatInt(9))
            case .nineBySixteen:
                return String(format: AspectRatio.aspectRatioXByYFormatString(), SecondCameraEditorFormat.formatInt(9), SecondCameraEditorFormat.formatInt(16))
            }
        }
    }
...
extension SecondCameraImageEditorCropViewController {
    ...
    @objc
    private func didTapSecondCameraCancel() {
        transitionSecondCameraEditorUI(toState: .initial, animated: true) { finished in
            guard finished else { return }
            self.dismissSecondCameraCropEditor()
        }
    }

    @objc
    private func didTapSecondCameraDone() {
    ...
    private func dismissSecondCameraCropEditor() {
    ...
    private func didTapReset() {
        let newTransform = SecondCameraImageEditorTransform.defaultTransform(srcImageSizePixels: model.srcImageSizePixels)
        applyTransformWithAnimation(newTransform)
    }

    @objc
    private func didTapChooseAspectRatio() {
        let actionSheet = SecondCameraEditorActionSheetController()
        ...
        for aspectRatio in AspectRatio.allCases {
            ...
            actionSheet.addAction(
                SecondCameraEditorActionSheetAction(
                    ...
                    handler: { [weak self] _ in
                        ...
                        SparkLogger.log(
                            level: .info,
                            module: .camera,
                            message: "SecondCamera crop selected aspectRatio=\(aspectRatio)"
                        )
        ...
        actionSheet.addAction(SecondCameraEditorActionSheetAction(title: SecondCameraEditorCommonStrings.cancelButton, style: .cancel))
        SparkLogger.log(
            level: .info,
            module: .camera,
            message: "SecondCamera crop aspect ratio sheet actionCount=\(actionSheet.actions.count)"
        )
...
extension SecondCameraImageEditorCropViewController {
    ...
    private func setupSecondCameraRotationControlActions() {
    ...
    @objc
    private func rotationControlValueChanged(_ sender: SecondCameraRotationControl) {
    ...
    @objc
    private func rotationControlDidBeginEditing(_ sender: SecondCameraRotationControl) {
        setCropFrameGridLines(hidden: false, animated: true)
    }

    @objc
    private func rotationControlDidEndEditing(_ sender: SecondCameraRotationControl) {
...
extension SecondCameraImageEditorCropViewController: UIGestureRecognizerDelegate {
