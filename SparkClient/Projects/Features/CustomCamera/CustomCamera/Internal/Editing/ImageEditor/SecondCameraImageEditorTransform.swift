...
//
// The "SecondCameraImageEditorTransform" can be used to generate an CGAffineTransform
...
nonisolated class SecondCameraImageEditorTransform: NSObject {
    ...
    class func defaultTransform(srcImageSizePixels: CGSize) -> SecondCameraImageEditorTransform {
        // It shouldn't be necessary normalize the default transform, but we do so to be safe.
        return SecondCameraImageEditorTransform(
    ...
    var isNonDefault: Bool {
        return !isEqual(SecondCameraImageEditorTransform.defaultTransform(srcImageSizePixels: outputSizePixels))
    }
    ...
    func normalize(srcImageSizePixels: CGSize) -> SecondCameraImageEditorTransform {
        // Normalize scaling.
        ...
        let naiveImageFrameCanvas = SecondCameraImageEditorCanvasView.imageFrame(forViewSize: viewBounds.size, imageSize: srcImageSizePixels, transform: naiveTransform)
    ...
    override func isEqual(_ object: Any?) -> Bool {
        guard let other = object as? SecondCameraImageEditorTransform else {
            return false
        }
