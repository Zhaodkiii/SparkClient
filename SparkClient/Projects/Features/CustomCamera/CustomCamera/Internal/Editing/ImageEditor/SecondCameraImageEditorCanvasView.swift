...
// MARK: - SecondCameraEditorEditorTextLayer

class SecondCameraEditorEditorTextLayer: CATextLayer {
...
// MARK: - SecondCameraEditorTextFrameLayer
...
// MARK: - SecondCameraImageEditorCanvasView

// A view for previewing an image editor model.
class SecondCameraImageEditorCanvasView: UIView {
        ...
        didSet {
            if let itemId = oldValue, let layer = contentLayerMap[itemId] {
                ...
                if itemId == selectedTransformableItemID {
                    selectedSecondCameraEditorTextFrameLayer?.isHidden = false
                }
            }
            if let hiddenItemId, let layer = contentLayerMap[hiddenItemId] {
                ...
                if hiddenItemId == selectedTransformableItemID {
                    selectedSecondCameraEditorTextFrameLayer?.isHidden = true
                }
            }
        }
    }
    ...
    init(model: SecondCameraImageEditorModel, hiddenItemId: String? = nil) {
        self.model = model
    ...
    private func updateLayout() {
        ...
        contentViewConstraints = SecondCameraImageEditorCanvasView.updateContentLayout(
    ...
    class func updateContentLayout(
        transform: SecondCameraImageEditorTransform,
        contentView: UIView,
    ) -> [NSLayoutConstraint] {
        guard let superview = contentView.superview else {
            secondCameraEditorFailDebug("Content view has no superview.")
            return []
        }
    ...
    class func loadSrcImage(model: SecondCameraImageEditorModel) -> UIImage? {
        let srcImageData: Data
        do {
            srcImageData = try model.srcImage.dataSource.readData()
        } catch {
            secondCameraEditorFailDebug("Couldn't parse srcImageUrl")
            return nil
        }
        ...
        guard let srcImage = UIImage(data: srcImageData, scale: 1.0) else {
            secondCameraEditorFailDebug("Couldn't load background image.")
            return nil
        }
    ...
    private var selectedSecondCameraEditorTextFrameLayer: SecondCameraEditorTextFrameLayer?
    private lazy var tooltipView: UIView = {
        let hMargin: CGFloat = 9
        let vMargin: CGFloat = 5
        let label = UILabel()
        let text = NSAttributedString(
            string: SecondCameraEditorLocalizedString(
            ...
            attributes: [
                ...
                .foregroundColor: UIColor.secondCameraEditor_white,
            ],
        )
        ...
        background.backgroundColor = .secondCameraEditor_blackAlpha40
    ...
    private func updateSelectedTransformableItemFrame() {
        guard
            ...
            selectedSecondCameraEditorTextFrameLayer?.removeFromSuperlayer()
            selectedSecondCameraEditorTextFrameLayer = nil
            return
        }

        let selectedSecondCameraEditorTextFrameLayer = selectedSecondCameraEditorTextFrameLayer ?? SecondCameraEditorTextFrameLayer()
        if selectedSecondCameraEditorTextFrameLayer.superlayer == nil {
            contentView.layer.addSublayer(selectedSecondCameraEditorTextFrameLayer)
            selectedSecondCameraEditorTextFrameLayer.zPosition = SecondCameraImageEditorCanvasView.selectionFrameLayerZ
            self.selectedSecondCameraEditorTextFrameLayer = selectedSecondCameraEditorTextFrameLayer
        }
        ...
        selectedSecondCameraEditorTextFrameLayer.bounds = textLayer.bounds
            .inset(by: SecondCameraImageEditorCanvasView.textFrameInsets)
            .applying(CGAffineTransform(scaleX: scaleX, y: scaleY))
        selectedSecondCameraEditorTextFrameLayer.position = textLayer.position
        selectedSecondCameraEditorTextFrameLayer.setAffineTransform(CGAffineTransform(rotationAngle: rotationAngle))
        selectedSecondCameraEditorTextFrameLayer.layoutSublayers()

        selectedSecondCameraEditorTextFrameLayer.opacity = shouldFadeTransformableItem ? 0.5 : 1
    ...
    func updateTooltip() {
        guard
            let selectedItemID = selectedTransformableItemID,
            let stickerItem = model.item(forId: selectedItemID) as? SecondCameraImageEditorStickerItem,
            case .story = stickerItem.sticker,
            let stickerLayer = contentLayerMap[selectedItemID],
            let selectedSecondCameraEditorTextFrameLayer
        else {
            hideTooltip()
            return
        }
        ...
        if tooltipView.superview == nil {
            ...
            tooltipView.layer.zPosition = SecondCameraImageEditorCanvasView.selectionFrameLayerZ
        }
        ...
        tooltipView.layer.position = stickerLayer.position.offsetBy(
            dx: 0,
            dy: -(selectedSecondCameraEditorTextFrameLayer.bounds.height / 2 + tooltipView.frame.height / 2 + 8),
        )
    }
    ...
    private func updateAllContent() {
            ...
            viewSize.height > 0
        {
            ...
            for item in model.items() {
                guard
                    let layer = SecondCameraImageEditorCanvasView.layerForItem(
                        item: item,
                        model: model,
    ...
    private func updateImageLayer() {
        let viewSize = clipView.bounds.size
        SecondCameraImageEditorCanvasView.updateImageLayer(
    ...
    nonisolated class func imageFrame(forViewSize viewSize: CGSize, imageSize: CGSize, transform: SecondCameraImageEditorTransform) -> CGRect {
        ...
        guard imageSize.width > 0, imageSize.height > 0 else {
            secondCameraEditorFailDebug("Invalid imageSize")
            return .zero
        }
    ...
    private class func imageLayerForItem(
        ...
        guard let srcImage = loadSrcImage(model: model) else {
            secondCameraEditorFailDebug("Could not load src image.")
            return nil
        }
    ...
    private class func layerForItem(
        ...
        case .test:
            secondCameraEditorFailDebug("Unexpected test item.")
            return nil
        case .stroke:
            guard let strokeItem = item as? SecondCameraImageEditorStrokeItem else {
                secondCameraEditorFailDebug("Item has unexpected type: \(type(of: item)).")
                return nil
            }
        ...
        case .text:
            guard let textItem = item as? SecondCameraImageEditorTextItem else {
                secondCameraEditorFailDebug("Item has unexpected type: \(type(of: item)).")
                return nil
            }
        ...
        case .sticker:
            guard let stickerItem = item as? SecondCameraImageEditorStickerItem else {
                secondCameraEditorFailDebug("Item has unexpected type: \(type(of: item)).")
                return nil
            }
            let isFaded = item.itemId == fadedItemID
        ...
        case .blurRegions:
            guard let blurRegionsItem = item as? SecondCameraImageEditorBlurRegionsItem else {
                secondCameraEditorFailDebug("Item has unexpected type: \(type(of: item)).")
                return nil
            }
    ...
    private class func strokeLayerForItem(
        item: SecondCameraImageEditorStrokeItem,
        model: SecondCameraImageEditorModel,
        transform: SecondCameraImageEditorTransform,
        viewSize: CGSize,
    ) -> CALayer? {
        ...
        if item.strokeType == .blur {
            guard let blurredImageLayer = blurredImageLayerForItem(model: model, transform: transform, viewSize: viewSize) else {
                secondCameraEditorFailDebug("Failed to retrieve blurredImageLayer")
                return nil
            }
        ...
        let imageFrame = SecondCameraImageEditorCanvasView.imageFrame(forViewSize: viewSize, imageSize: model.srcImageSizePixels, transform: transform)
        ...
        for index in 0..<points.count {
            ...
            if points.count <= 1 {
                ...
                forwardVector = CGPoint.secondCameraEditor_subtract(nextPoint, point)
            } else if index == points.count - 1 {
                // Last sample.
                let previousPoint = points[index - 1]
                forwardVector = CGPoint.secondCameraEditor_subtract(point, previousPoint)
            } else {
                // Middle samples.
                let previousPoint = points[index - 1]
                let previousPointForwardVector = CGPoint.secondCameraEditor_subtract(point, previousPoint)
                let nextPoint = points[index + 1]
                let nextPointForwardVector = CGPoint.secondCameraEditor_subtract(nextPoint, point)
                forwardVector = CGPoint.secondCameraEditor_scale(CGPoint.secondCameraEditor_add(previousPointForwardVector, nextPointForwardVector), factor: 0.5)
            }
            ...
            if index == 0 {
                ...
                let controlPoint1 = CGPoint.secondCameraEditor_add(previousPoint, CGPoint.secondCameraEditor_scale(previousForwardVector, factor: +controlPointFactor))
                let controlPoint2 = CGPoint.secondCameraEditor_add(point, CGPoint.secondCameraEditor_scale(forwardVector, factor: -controlPointFactor))
        ...
        if item.strokeType == .blur {
            guard let blurredImageLayer = optionalBlurredImageLayer else {
                secondCameraEditorFailDebug("Unexpectedly missing blurredImageLayer")
                return nil
            }
    ...
    private class func blurRegionsLayerForItem(
        item: SecondCameraImageEditorBlurRegionsItem,
        model: SecondCameraImageEditorModel,
        transform: SecondCameraImageEditorTransform,
        viewSize: CGSize,
    ) -> CALayer? {
    ...
    private class func zPositionForItem(
        ...
        guard let itemIndex = itemIds.firstIndex(of: item.itemId) else {
            secondCameraEditorFailDebug("Couldn't find index of item.")
    ...
    private class func textLayerForItem(
        item: SecondCameraImageEditorTextItem,
        model: SecondCameraImageEditorModel,
        isFaded: Bool,
        transform: SecondCameraImageEditorTransform,
        viewSize: CGSize,
        ...
        let font = SecondCameraMediaTextView.font(for: item.textStyle, withPointSize: fontSize)

        let text = item.text.secondCameraEditor_filterForDisplay
            ...
            case .underline:
                textStorage.addAttributes(
                    [
                        ...
                        NSAttributedString.Key.underlineColor: textDecorationColor,
                    ],
                    range: textStorage.ows_entireRange,
                )
            ...
            case .outline:
                textStorage.addAttributes(
                    [
                        ...
                        NSAttributedString.Key.strokeColor: textDecorationColor,
                    ],
                    range: textStorage.ows_entireRange,
                )
        ...
        let background = item.textBackgroundColor.map { color in
            SecondCameraImageEditorItemBackground(color: color, corners: .small, scaleFactor: scaleFactor)
        }
    ...
    private class func textLayer(
        ...
        imageFrame: CGRect,
        unitWidth: CGFloat = 1,
        background: SecondCameraImageEditorItemBackground?,
        item: any SecondCameraImageEditorTransformable,
        model: SecondCameraImageEditorModel,
        isFaded: Bool,
        transform: SecondCameraImageEditorTransform,
        viewSize: CGSize,
    ) -> CALayer? {
        let textLayer = SecondCameraEditorEditorTextLayer(itemId: item.itemId)
        ...
        let layer: SecondCameraEditorEditorTextLayer
        if let background {
    ...
    private class func stickerLayerForItem(
        item: SecondCameraImageEditorStickerItem,
        model: SecondCameraImageEditorModel,
        isFaded: Bool,
        transform: SecondCameraImageEditorTransform,
        viewSize: CGSize,
        ...
        case (.none, .regular(let stickerInfo)):
            ...
            guard
                let metadata = SecondCameraStickerManager.installedStickerMetadataWithSneakyTransaction(stickerInfo: stickerInfo),
                let imageData = try? metadata.readStickerData(),
                let stickerImage = UIImage(data: imageData)
            else {
                secondCameraEditorFailDebug("Failed to retrieve sticker image")
                return nil
            }
            model.stickerViewCache.setObject(SecondCameraEditorThreadSafeCacheHandle(stickerImage), forKey: item.itemId)
            image = stickerImage
    ...
    private class func storyStickerLayer(
        for storySticker: SecondCameraEditorSticker.StorySticker,
        imageFrame: CGRect,
        item: SecondCameraImageEditorStickerItem,
        model: SecondCameraImageEditorModel,
        isFaded: Bool,
        ...
        case .clockDigital(let digitalClockStyle):
            ...
            let background = digitalClockStyle.backgroundColor.map { color in
                SecondCameraImageEditorItemBackground(color: color, corners: .large, scaleFactor: scaleFactor)
            }
    ...
    private class func applySmoothing(to points: [CGPoint]) -> [CGPoint] {
        ...
        for index in 0..<points.count {
            ...
            if index == 0 {
                ...
                let smoothedPoint = CGPoint.secondCameraEditor_add(
                    CGPoint.secondCameraEditor_scale(point, factor: 1.0 - 2.0 * alpha),
                    CGPoint.secondCameraEditor_add(
                        CGPoint.secondCameraEditor_scale(lastPoint, factor: alpha),
                        CGPoint.secondCameraEditor_scale(nextPoint, factor: alpha),
                    ),
                )
    ...
    private func prepareBlurredImage() {
        guard let srcImage = loadSrcImage() else {
            return secondCameraEditorFailDebug("Could not load src image.")
        }
        ...
        Task { @MainActor [weak self] in
            do {
                ...
                secondCameraEditorFailDebug("Failed to blur src image")
            }
        }
    }
    ...
    class func renderForOutput(model: SecondCameraImageEditorModel, transform: SecondCameraImageEditorTransform) -> UIImage? {
        ...
        for item in model.items() {
            guard
                ...
                secondCameraEditorFailDebug("Couldn't create layer for item.")
                continue
            }
        ...
        let image = view.renderAsImage(opaque: !model.srcSecondCameraEditorImageMetadata.hasAlpha, scale: dstScale)
        return image
    }
    ...
    func transformableLayer(forLocation point: CGPoint) -> CALayer? {
        ...
        if
            ...
            let selectedTextLayer = contentLayerMap[selectedTextItemId] as? SecondCameraEditorEditorTextLayer,
            selectedSecondCameraEditorTextFrameLayer.hitTest(point) != nil
        {
        ...
        return model.items()
            .lazy
            .reversed()
            .filter { $0 is SecondCameraImageEditorTransformable }
...
extension SecondCameraImageEditorCanvasView: SecondCameraImageEditorModelObserver {
