...
import UIKit
struct SecondCameraImageEditorPinchState {
    let centroid: CGPoint
    let distance: CGFloat
    let angleRadians: CGFloat

    static var empty: SecondCameraImageEditorPinchState {
        return SecondCameraImageEditorPinchState(centroid: .zero, distance: 1.0, angleRadians: 0)
    }
}
...
class SecondCameraImageEditorPinchGestureRecognizer: UIGestureRecognizer {

    weak var referenceView: UIView?

    var pinchStateStart = SecondCameraImageEditorPinchState.empty

    var pinchStateLast = SecondCameraImageEditorPinchState.empty
    ...
    enum TouchState {
        case possible
        case valid(pinchState: SecondCameraImageEditorPinchState)
        case invalid
    }

    private func touchState(for event: UIEvent) -> TouchState {
        guard let allTouches = event.allTouches else {
            secondCameraEditorFailDebug("Missing allTouches")
            return .invalid
        }
    ...
    private func pinchState() -> SecondCameraImageEditorPinchState? {
        guard let referenceView else {
            secondCameraEditorFailDebug("Missing view")
            return nil
        }
        ...
        let centroid = CGPoint.secondCameraEditor_scale(CGPoint.secondCameraEditor_add(location0, location1), factor: 0.5)
        ...
        let delta = CGPoint.secondCameraEditor_subtract(location1, location0)
        let angleRadians = atan2(delta.y, delta.x)
        return SecondCameraImageEditorPinchState(
            centroid: centroid,
            distance: distance,
    ...
    private func centroid(forTouches touches: Set<UITouch>?) -> CGPoint {
        ...
        for touch in touches {
            ...
            sum = CGPoint.secondCameraEditor_add(sum, location)
        }

        let centroid = CGPoint.secondCameraEditor_scale(sum, factor: 1 / CGFloat(touches.count))
        return centroid
    }
}
