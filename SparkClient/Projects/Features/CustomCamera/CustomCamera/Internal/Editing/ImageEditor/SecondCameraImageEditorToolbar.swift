...
class SecondCameraImageEditorTopBar: SecondCameraMediaTopBar {

    let undoButton = SecondCameraRoundMediaButton(
    ...
    let clearAllButton = SecondCameraRoundMediaButton(image: nil, backgroundStyle: .blur)
    ...
    override init(frame: CGRect) {
        ...
        let clearAllButtonTitle =
            SecondCameraEditorLocalizedString(
        ...
        clearAllButton.secondCameraEditor_contentEdgeInsets = UIEdgeInsets(hMargin: 26, vMargin: 15)
...
protocol SecondCameraImageEditorBottomBarButtonProvider: AnyObject {

    var middleButtons: [UIButton] { get }
}

protocol SecondCameraImageEditorBottomBarProvider: AnyObject {

    func bottomBar(for viewController: UIViewController) -> SecondCameraImageEditorBottomBar
}

class SecondCameraImageEditorBottomBar: UIView {

    let cancelButton: UIButton = SecondCameraRoundMediaButton(
        image: UIImage(imageLiteralResourceName: "x-28"),
        backgroundStyle: .solid(SecondCameraRoundMediaButton.defaultBackgroundColor),
    )
    let doneButton: UIButton = SecondCameraRoundMediaButton(
    ...
    init(buttonProvider: SecondCameraImageEditorBottomBarButtonProvider?) {
