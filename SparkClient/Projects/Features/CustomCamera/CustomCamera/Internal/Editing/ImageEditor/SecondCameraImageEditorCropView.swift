...
enum SecondCameraImageEditorCropRegion {
...
private class SecondCameraEditorCropCornerView: UIView {

    let cropRegion: SecondCameraImageEditorCropRegion

    var size: CGSize = CGSize(square: SecondCameraEditorCropView.desiredCornerSize) {
        didSet {
            cornerWidthConstraint.constant = size.width
            cornerHeightConstraint.constant = size.height
        }
    }

    private lazy var cornerWidthConstraint: NSLayoutConstraint = self.widthAnchor.constraint(equalToConstant: size.width)
    private lazy var cornerHeightConstraint: NSLayoutConstraint = self.heightAnchor.constraint(equalToConstant: size.width)

    init(cropRegion: SecondCameraImageEditorCropRegion) {
        self.cropRegion = cropRegion
        super.init(frame: .zero)
        isUserInteractionEnabled = false
        translatesAutoresizingMaskIntoConstraints = false
        shapeLayer?.fillColor = UIColor.white.cgColor
        addConstraints([cornerWidthConstraint, cornerHeightConstraint])
    }
    ...
    private func updatePath() {
        ...
        default:
            secondCameraEditorFailDebug("Invalid crop region: \(cropRegion)")
        }
...
private class SecondCameraEditorCropBackgroundView: UIView {
...
class SecondCameraEditorCropView: UIView {

    static let desiredCornerSize: CGFloat = 22 // adjusted for stroke width, visible size is 24
    private(set) var cornerSize = CGSize(square: SecondCameraEditorCropView.desiredCornerSize)

    private lazy var backgroundView = SecondCameraEditorCropBackgroundView(style: SecondCameraEditorCropView.backgroundStyle(forState: state))
    ...
    private let cropCornerViews: [SecondCameraEditorCropCornerView] = [
        SecondCameraEditorCropCornerView(cropRegion: .topLeft),
        SecondCameraEditorCropCornerView(cropRegion: .topRight),
        SecondCameraEditorCropCornerView(cropRegion: .bottomLeft),
        SecondCameraEditorCropCornerView(cropRegion: .bottomRight),
    ]
    ...
    override init(frame: CGRect) {
        ...
        for cropCornerView in cropCornerViews {
            ...
            default:
                secondCameraEditorFailDebug("Invalid crop region: \(String(describing: cropCornerView.cropRegion))")
            }
            switch cropCornerView.cropRegion {
        ...
        for (index, line) in verticalGridLines.enumerated() {
            line.backgroundColor = .secondCameraEditor_white
    ...
    func setState(_ state: State, animated: Bool, completion: ((Bool) -> Void)? = nil) {
        ...
        let backgroundStyle = SecondCameraEditorCropView.backgroundStyle(forState: state)
        let layoutBlock = {
    ...
    private class func backgroundStyle(forState state: State) -> SecondCameraEditorCropBackgroundView.Style {
        switch state {
    ...
    private func updateCornerSize() {
        ...
        self.cornerSize = CGSize(
            width: min(cropFrameView.width * 0.5, SecondCameraEditorCropView.desiredCornerSize),
            height: min(cropFrameView.height * 0.5, SecondCameraEditorCropView.desiredCornerSize),
        )
...
private extension UIBezierPath {
    func addRegion(withPoints points: [CGPoint]) {
        guard let first = points.first else {
            secondCameraEditorFailDebug("No points.")
            return
        }
        move(to: first)
