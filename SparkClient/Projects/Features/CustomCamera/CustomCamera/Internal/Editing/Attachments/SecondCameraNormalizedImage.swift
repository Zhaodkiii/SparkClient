...
/// Represents ``SecondCameraPreviewableAttachment``s that are images. This is roughly
...
public struct SecondCameraNormalizedImage {
    let dataSource: SecondCameraEditorDataSourcePath
    let dataUTI: String
    ...
    private static func loadImage(dataSource: SecondCameraEditorDataSourcePath, maxPixelSize: CGFloat) throws(SecondCameraAttachmentError) -> CGImage {
    ...
    private static func saveImage(_ image: CGImage, containerType: ContainerType) throws(SecondCameraAttachmentError) -> SecondCameraEditorDataSourcePath {
        let tempFileUrl = SecondCameraEditorFileSystem.temporaryFileUrl(
        ...
        return SecondCameraEditorDataSourcePath(fileUrl: tempFileUrl, ownership: .owned)
    }
    ...
    public static func forImage(
        ...
        guard let imageData = image.pngData() else {
            throw SecondCameraAttachmentError.couldNotConvertImage
        }
        SecondCameraEditorLogger.info("creating forImage")
        let dataSource = try SecondCameraEditorDataSourcePath(writingTempFileData: imageData, fileExtension: containerType.fileExtension)
        ...
        return try forSecondCameraEditorDataSource(
            dataSource,
    ...
    static func forSecondCameraEditorDataSource(
        _ dataSource: SecondCameraEditorDataSourcePath,
        dataUTI: String,
        ...
        let imageQuality = SecondCameraEditorImageQualityLevel.maximumForSecondCameraEditorCurrentRuntime()
        ...
        let originalMightBeValid = { () -> Bool in
            guard SecondCameraAttachment.outputImageUTISet.contains(dataUTI) else {
                return false
            }
            guard let imageMetadata, imageMetadata.pixelSize.secondCameraEditor_largerAxis <= imageQuality.startingTier.maxEdgeSize else {
                return false
            }
            return true
        }()

        SecondCameraEditorLogger.info("creating forSecondCameraEditorDataSource (originalMightBeValid: \(originalMightBeValid))")

        let normalizedSecondCameraEditorDataSource: SecondCameraEditorDataSourcePath
        ...
        if originalMightBeValid {
            ...
            normalizedSecondCameraEditorDataSource = dataSource
            ...
            (normalizedSecondCameraEditorDataSource, normalizedDataUTI) = try normalizingSecondCameraEditorDataSource(
                dataSource,
        ...
        return Self(
            dataSource: normalizedSecondCameraEditorDataSource,
    ...
    private static func normalizingSecondCameraEditorDataSource(
        _ dataSource: SecondCameraEditorDataSourcePath,
        imageQuality: SecondCameraEditorImageQualityLevel,
    ) throws(SecondCameraAttachmentError) -> (dataSource: SecondCameraEditorDataSourcePath, dataUTI: String) {
        let tier = imageQuality.startingTier
        return try autoreleasepool { () throws(SecondCameraAttachmentError) -> (dataSource: SecondCameraEditorDataSourcePath, dataUTI: String) in
            let containerType = ContainerType.png
            let cgImage = try loadImage(dataSource: dataSource, maxPixelSize: tier.maxEdgeSize)
            let outputSecondCameraEditorDataSource = try saveImage(cgImage, containerType: containerType)
            outputSecondCameraEditorDataSource.sourceFilename = Self.replaceFileExtension(
                sourceFilename: dataSource.sourceFilename,
                newFileExtension: containerType.fileExtension,
            )
            return (outputSecondCameraEditorDataSource, containerType.dataType.identifier)
        }
    }
    ...
    func finalizeImage(imageQuality: SecondCameraEditorImageQualityLevel) throws -> FinalizedImage {
        SecondCameraEditorLogger.info("finalizing (mustCompress: \(mustCompress))")
        if !mustCompress {
            // When constructing a SecondCameraNormalizedImage, we check if the original image
            ...
            SecondCameraEditorLogger.info("finalizing (isOriginalStillValid: \(isOriginalStillValid))")
            if isOriginalStillValid {
                SecondCameraEditorLogger.info("finalizing (mayHaveMetadata: \(mayHaveMetadata))")
                if !mayHaveMetadata {
                    return FinalizedImage(dataSource: dataSource, dataUTI: dataUTI)
                }
                let strippedSecondCameraEditorDataSource = try stripImage()
                SecondCameraEditorLogger.info("finalizing (strippedSecondCameraEditorDataSource != nil: \(strippedSecondCameraEditorDataSource != nil))")
                if let strippedSecondCameraEditorDataSource {
                    return FinalizedImage(dataSource: strippedSecondCameraEditorDataSource, dataUTI: dataUTI)
                }
            }
        }
    ...
    private func stripImage() throws -> SecondCameraEditorDataSourcePath? {
        // If we can't strip it, we can fall back to compressing it.
        let strippedData = try? Self.removeSecondCameraEditorImageMetadata(fromData: dataSource.readData(), dataUti: dataUTI)
        ...
        guard let fileExtension = SecondCameraEditorMimeTypeUtil.fileExtensionForUtiType(dataUTI) else {
            throw SecondCameraAttachmentError.couldNotRemoveMetadata
        }
        let dataSource = try SecondCameraEditorDataSourcePath(writingTempFileData: strippedData, fileExtension: fileExtension)
    ...
    private func compressImageToQuality(_ imageQuality: SecondCameraEditorImageQualityLevel) throws -> FinalizedImage {
        var nextTier: SecondCameraEditorImageQualityTier? = imageQuality.startingTier
        while let currentTier = nextTier {
            SecondCameraEditorLogger.info("compressing to tier \(currentTier.rawValue)")
        ...
        throw SecondCameraAttachmentError.fileSizeTooLarge
    }

    /// Compress the image to the specified tier.
    private func compressImageToTier(_ tier: SecondCameraEditorImageQualityTier) throws(SecondCameraAttachmentError) -> FinalizedImage {
        ...
        let outputSecondCameraEditorDataSource = try Self.saveImage(cgImage, containerType: containerType)
        outputSecondCameraEditorDataSource.sourceFilename = Self.replaceFileExtension(
            sourceFilename: dataSource.sourceFilename,
            newFileExtension: containerType.fileExtension,
        )
        return FinalizedImage(
            dataSource: outputSecondCameraEditorDataSource,
            dataUTI: containerType.dataType.identifier,
        )
    }
    ...
    private static func compressionQuality(for pixelSize: CGSize) -> CGFloat {
        ...
        if pixelSize.secondCameraEditor_largerAxis >= 3072 { return 0.55 }
        return 0.6
    }
    ...
    nonisolated(unsafe) private static let preservedMetadata: [CFString] = [
    ...
    private static func removeSecondCameraEditorImageMetadata(fromData dataValue: Data, dataUti: String) throws(SecondCameraAttachmentError) -> Data {
        if dataUti == UTType.png.identifier {
            return try self.removeSecondCameraEditorImageMetadata(fromPngData: dataValue)
        } else {
            return try self.removeSecondCameraEditorImageMetadata(fromNonPngData: dataValue)
        }
    }

    /// Remove nonessential chunks from PNG data.
    /// - Returns: Cleaned PNG data.
    /// - Throws: `SecondCameraAttachmentError.couldNotRemoveMetadata` if the PNG parser fails.
    static func removeSecondCameraEditorImageMetadata(fromPngData pngData: Data) throws(SecondCameraAttachmentError) -> Data {
        do {
            let chunker = try SecondCameraEditorPngChunker(source: SecondCameraEditorDataImageSource(pngData))
            var result = SecondCameraEditorPngChunker.pngSignature
            ...
            SecondCameraEditorLogger.warn("Could not remove PNG metadata: \(error)")
            throw .couldNotRemoveMetadata
        }
    }

    private static func removeSecondCameraEditorImageMetadata(fromNonPngData dataValue: Data) throws(SecondCameraAttachmentError) -> Data {
        ...
        guard (type as String) != UTType.jpeg.identifier else {
            SecondCameraEditorLogger.warn("falling back to compression for JPEG")
            throw .couldNotRemoveMetadata
        }
