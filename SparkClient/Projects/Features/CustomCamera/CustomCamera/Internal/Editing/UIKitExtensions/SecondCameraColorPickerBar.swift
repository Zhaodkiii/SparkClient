...
public protocol SecondCameraColorPickerBarViewDelegate: AnyObject {
...
public class SecondCameraColorPickerBarColor {
    ...
    class func defaultColor() -> SecondCameraColorPickerBarColor {
        return SecondCameraColorPickerBarColor(color: UIColor(rgbHex: 0xff0000), palettePhase: 1 / 9)
    }

    class var white: SecondCameraColorPickerBarColor {
        SecondCameraColorPickerBarColor(color: .white, palettePhase: 1)
    }

    class var black: SecondCameraColorPickerBarColor {
        SecondCameraColorPickerBarColor(color: .black, palettePhase: 0)
    }
    ...
    static func ==(left: SecondCameraColorPickerBarColor, right: SecondCameraColorPickerBarColor) -> Bool {
        return left.palettePhase.secondCameraEditor_fuzzyEquals(right.palettePhase)
    }
}

// MARK: -

private class SecondCameraEditorColorPreviewView: SecondCameraEditorLayerView {
    ...
    override init() {
        ...
        layoutCallback = { view in
            SecondCameraEditorColorPreviewView.updateLayers(
                view: view,
        ...
        autoSetDimensions(to: CGSize(square: SecondCameraEditorColorPreviewView.innerRadius * 4))
    }
    ...
    static func updateLayers(
        ...
        let startAngle = angle + .secondCameraEditor_halfPi
        let endAngle = -angle + .secondCameraEditor_halfPi
...
public class SecondCameraColorPickerBarView: UIView {

    public weak var delegate: SecondCameraColorPickerBarViewDelegate?

    public var color: UIColor { selectedValue.color }
    var selectedValue: SecondCameraColorPickerBarColor {
        didSet {
            updateState()
        }
    }

    init(currentColor: SecondCameraColorPickerBarColor? = nil) {
        selectedValue = currentColor ?? SecondCameraColorPickerBarColor.defaultColor()
    ...
    private var selectionView: UIView = {
        let selectionView = SecondCameraCircleView(diameter: selectionSize)
        ...
        let borderView = SecondCameraCircleView(diameter: selectionSize + 1)
    ...
    private let imageWrapper = SecondCameraEditorLayerView()
    private var selectionConstraint: NSLayoutConstraint?
    private let previewView = SecondCameraEditorColorPreviewView()
    ...
    private func createContents() {
        ...
        let image = SecondCameraColorPickerBarView.buildPaletteGradientImage()
        imageView.image = image
        let imageRadius = image.size.height * 0.5
        imageView.layer.cornerRadius = imageRadius
        imageView.clipsToBounds = true
        addSubview(imageView)
        imageView.autoSetDimension(.height, toSize: SecondCameraColorPickerBarView.colorBarWidth)
        ...
        let imageViewBorder = SecondCameraPillView()
        ...
        addGestureRecognizer(SecondCameraPermissiveGestureRecognizer(target: self, action: #selector(didTouch)))

        updateState()
    }

    private func selectColor(atLocationX locationX: CGFloat) {
        let palettePhase = locationX.secondCameraEditor_inverseLerp(0, imageView.width, shouldClamp: true)
        selectedValue = value(for: palettePhase)

        delegate?.secondCameraColorPickerBarView(self, didSelectColor: selectedValue)
    }

    private func value(for palettePhase: CGFloat) -> SecondCameraColorPickerBarColor {
        ...
        let segmentCount = SecondCameraColorPickerBarColor.gradientUIColors.count - 1
        var prevColor: UIColor?
        for color in SecondCameraColorPickerBarColor.gradientUIColors {
        ...
        guard let segment = bestSegment else {
            secondCameraEditorFailDebug("Couldn't find matching segment.")
            return SecondCameraColorPickerBarColor.defaultColor()
        }
        guard
            palettePhase >= segment.palettePhase0,
            palettePhase <= segment.palettePhase1
        else {
            secondCameraEditorFailDebug("Invalid segment.")
            return SecondCameraColorPickerBarColor.defaultColor()
        }
        let segmentPhase = palettePhase.secondCameraEditor_inverseLerp(segment.palettePhase0, segment.palettePhase1).secondCameraEditor_clamp01()
        ...
        return SecondCameraColorPickerBarColor(color: color, palettePhase: palettePhase)
    }
    ...
    private func updateState() {
        ...
        guard let selectionConstraint else {
            secondCameraEditorFailDebug("Missing selectionConstraint.")
            return
        }
    ...
    private static func buildPaletteGradientImage() -> UIImage {
        ...
        gradientLayer.colors = SecondCameraColorPickerBarColor.gradientCGColors
