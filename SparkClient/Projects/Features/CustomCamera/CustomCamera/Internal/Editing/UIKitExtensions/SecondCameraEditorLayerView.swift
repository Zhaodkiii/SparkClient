...
open class SecondCameraEditorLayerView: UIView {
    ...
    public static func circleView() -> SecondCameraEditorLayerView {
        circleView(size: nil)
    }

    public static func circleView(size: CGFloat? = nil) -> SecondCameraEditorLayerView {
        let result = SecondCameraEditorLayerView(frame: .zero) { view in
            view.layer.cornerRadius = min(view.width, view.height) * 0.5
        }
        if let size {
            result.autoSetDimensions(to: CGSize.secondCameraEditor_square(size))
        }
        return result
    }

    public static func pillView() -> SecondCameraEditorLayerView {
        pillView(height: nil)
    }

    public static func pillView(height: CGFloat? = nil) -> SecondCameraEditorLayerView {
        let result = SecondCameraEditorLayerView(frame: .zero) { view in
            view.layer.cornerRadius = min(view.width, view.height) * 0.5
        }
        if let height {
    ...
    private func didTap() {
        guard let tapBlock else {
            secondCameraEditorFailDebug("Missing tapBlock.")
            return
        }
        tapBlock()
    }
    ...
    private func didLongPress() {
        guard let longPressBlock else {
            secondCameraEditorFailDebug("Missing longPressBlock.")
            return
        }
        longPressBlock()
    }
