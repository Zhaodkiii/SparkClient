...
import UIKit
public class SecondCameraMediaTextView: UITextView {
    ...
    class func font(for textStyle: TextStyle, withPointSize pointSize: CGFloat) -> UIFont {
        let style: SecondCameraTextAttachment.TextStyle = {
            switch textStyle {
        ...
        return UIFont.secondCameraEditorFont(for: style, withPointSize: pointSize)
    }
    ...
    public func update(
        using textStylingToolbar: SecondCameraTextStylingToolbar,
        fontPointSize: CGFloat,
        textAlignment: NSTextAlignment = .center,
    ) {
        let font = SecondCameraMediaTextView.font(for: textStylingToolbar.textStyle, withPointSize: fontPointSize)
        updateWith(
...
public class SecondCameraTextStylingToolbar: UIControl {

    private let colorPickerView: SecondCameraColorPickerBarView

    // Photo Editor operates with SecondCameraColorPickerBarColor hence the need to expose this value.
    public var currentColorPickerValue: SecondCameraColorPickerBarColor {
        get { colorPickerView.selectedValue }
        set { colorPickerView.selectedValue = newValue }
    }

    public let textStyleButton = SecondCameraRoundMediaButton(
        image: SecondCameraTextStylingToolbar.buttonImage(forTextStyle: .regular),
        backgroundStyle: .blur,
    )
    public var textStyle: SecondCameraMediaTextView.TextStyle = .regular {
        didSet {
            textStyleButton.setImage(SecondCameraTextStylingToolbar.buttonImage(forTextStyle: textStyle), for: .normal)
        }
    }

    private static func buttonImage(forTextStyle textStyle: SecondCameraMediaTextView.TextStyle) -> UIImage? {
    ...
    public var textForegroundColor: UIColor {
        ...
        case .coloredBackground:
            ...
            return backgroundColor.secondCameraEditor_isCloseToColor(.white) ? .black : .white
    ...
    public var textBackgroundColor: UIColor? {
        ...
        case .whiteBackground:
            ...
            return textColor.secondCameraEditor_isCloseToColor(.white) ? .black : .white
    ...
    public let decorationStyleButton = SecondCameraRoundMediaButton(
        image: UIImage(imageLiteralResourceName: "text_effects"),
        backgroundStyle: .blur,
    )
    public var decorationStyle: SecondCameraMediaTextView.DecorationStyle = .none {
        didSet {
            decorationStyleButton.isSelected = (decorationStyle != .none)
        }
    }

    public lazy var doneButton = SecondCameraRoundMediaButton(image: SecondCameraEditorTheme.iconImage(.checkmark), backgroundStyle: .blur)
    ...
    public init(currentColor: SecondCameraColorPickerBarColor? = nil) {
        colorPickerView = SecondCameraColorPickerBarView(currentColor: currentColor ?? SecondCameraColorPickerBarColor.white)
        ...
        let contentWidthConstraint = stackViewLayoutGuide.widthAnchor.constraint(equalToConstant: SecondCameraImageEditorViewController.preferredToolbarContentWidth)
        ...
        if let button = stackView.arrangedSubviews.first as? SecondCameraRoundMediaButton {
            leadingMargin = button.layoutMargins.leading
        }
        if let button = stackView.arrangedSubviews.last as? SecondCameraRoundMediaButton {
...
extension SecondCameraTextStylingToolbar: SecondCameraColorPickerBarViewDelegate {

    public func secondCameraColorPickerBarView(_ pickerView: SecondCameraColorPickerBarView, didSelectColor color: SecondCameraColorPickerBarColor) {
