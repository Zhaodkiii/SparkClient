...
import UIKit
public enum SecondCameraEditorViewControllerLifecycle: Equatable {
...
open class SecondCameraEditorViewController: UIViewController {
    ...
    public private(set) final var lifecycle = SecondCameraEditorViewControllerLifecycle.notLoaded {
        didSet {
    ...
    public private(set) final var achievedLifecycleStates = Set<SecondCameraEditorViewControllerLifecycle>()
    ...
    deinit {
        ...
        SecondCameraEditorLogger.verbose("Dealloc: \(type(of: self))")
    }
    ...
    private func contentLayoutConstraintsForCurrentTraitCollection() -> [NSLayoutConstraint] {
        ...
        SecondCameraEditorLogger.debug("Vertical compact: [\(isVerticalCompact ? "Y" : "N")]")
        SecondCameraEditorLogger.debug("Horizontal compact: [\(isHorizontalCompact ? "Y" : "N")]")
        SecondCameraEditorLogger.debug("Layout margins: [\(view.layoutMarginsGuide.layoutFrame)]")
        ...
        if isVerticalCompact {
            ...
            if !isiPad, !UIDevice.current.secondCameraEditor_hasIPhoneXNotch {
                bottomMargin = 20
            }
            constraints += [
                contentLayoutGuide.topAnchor.constraint(equalTo: view.layoutMarginsGuide.topAnchor),
    ...
    override open var supportedInterfaceOrientations: UIInterfaceOrientationMask {
        return UIDevice.current.secondCameraEditor_defaultSupportedOrientations
    }
    ...
    private func handleKeyboardNotificationBase(_ notification: NSNotification) {
        ...
        guard let keyboardEndFrame = (userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue else {
            secondCameraEditorFailDebug("Missing keyboard end frame")
            return
        }
...
private class SecondCameraEditorPassthroughTouchSpacerView: UIView {
...
public extension SecondCameraEditorViewController {
