...
open class SecondCameraRoundMediaButton: UIButton {
    ...
    public enum BackgroundStyle {
        ...
        public nonisolated(unsafe) static let blur: BackgroundStyle = .blur(.dark)
        public nonisolated(unsafe) static let blurLight: BackgroundStyle = .blur(.light)
    }
    ...
    public static let defaultBackgroundColor = UIColor.secondCameraEditor_gray80
    static let visibleButtonSize: CGFloat = 42
    private static let defaultInset: CGFloat = UIDevice.current.secondCameraEditor_isNarrowerThanIPhone6 ? 4 : 8
    private static let defaultContentInset: CGFloat = UIDevice.current.secondCameraEditor_isNarrowerThanIPhone6 ? 11 : 15
    ...
    public init(image: UIImage?, backgroundStyle: BackgroundStyle, customView: UIView?) {
        ...
        super.init(frame: CGRect(origin: .zero, size: .secondCameraEditor_square(Self.visibleButtonSize + 2 * Self.defaultInset)))

        ows_contentEdgeInsets = UIEdgeInsets(margin: Self.defaultContentInset)
        layoutMargins = UIEdgeInsets(margin: Self.defaultInset)
        tintColor = SecondCameraEditorTheme.darkThemePrimaryColor
        insetsLayoutMarginsFromSafeArea = false

        setCompressionResistanceHigh()

        if backgroundView != nil || customView != nil {
            let backgroundContainerView = SecondCameraPillView()
