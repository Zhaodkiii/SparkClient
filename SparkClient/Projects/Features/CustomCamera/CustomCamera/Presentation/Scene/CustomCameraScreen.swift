//
//  Public+UI+CustomCameraScreen.swift of MijickCamera
//
...
/**
 ...
 - important: A view conforming to **CustomCameraScreen** has to be passed directly to ``CustomCameraView``. See ``CustomCameraView/setCameraScreen(_:)`` for more details.
 ...
 struct CustomCameraScreen: CustomCameraScreen {
 ...
 ```
 */
internal protocol CustomCameraScreen: View {
    var cameraManager: CustomCameraManager { get }
    var namespace: Namespace.ID { get }
    var closeCustomCameraAction: () -> () { get }
}

// MARK: Methods
internal extension CustomCameraScreen {
    /**
    ...
    func createCameraOutputView() -> some View {
        CameraBridgeView(cameraManager: cameraManager)
            .equatable()
            .reportCameraPreviewFrame()
    }
}
internal extension CustomCameraScreen {
    /**
    ...
    /**
     Set the camera zoom to a discrete preset.

     - note: Presets are derived from the current device capabilities.
     */
    func setZoomPreset(_ preset: CameraZoomPreset) throws { try cameraManager.setCameraZoomPreset(preset) }
...
internal extension CustomCameraScreen {
    ...
    var availableZoomPresets: [CameraZoomPreset] { cameraManager.availableZoomPresets }
...
internal extension CustomCameraScreen {
    ...
    var recordingTime: CustomCameraTime { cameraManager.videoOutput.recordingTime }
    ...
    var isCameraControlsVisible: Bool {
        cameraManager.attributes.error == nil
        && cameraManager.attributes.capturedMedia == nil
        && cameraManager.attributes.sessionState != .backgrounded
    }

    var isCameraControlsEnabled: Bool {
        cameraManager.attributes.isSessionReady
        && cameraManager.attributes.sessionState == .running
    }
}
