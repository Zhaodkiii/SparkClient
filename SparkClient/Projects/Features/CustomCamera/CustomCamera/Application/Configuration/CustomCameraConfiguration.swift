//
//  Public+CameraSettings+CustomCameraView.swift of MijickCamera
//
...
internal extension CustomCameraView {
...
internal extension CustomCameraView {
    /**
     ...
     For more details and tips on creating your own **Camera Screen**, see the ``CustomCameraScreen`` documentation.

     - tip: To hide selected buttons and controls on the screen, use the method with DefaultCustomCameraScreen as argument. For a code example, please refer to Usage -> Default Camera Screen Customization section.
    ...
    /**
     ...
     For more details and tips on creating your own **Captured Media Screen**, see the ``CustomCapturedMediaScreen`` documentation.
    ...
    /**
     ...
     For more details and tips on creating your own **Error Screen**, see the ``CustomCameraErrorScreen`` documentation.
...
internal extension CustomCameraView {
     ...
     */
    func setCameraOutputType(_ cameraOutputType: CameraOutputType) -> Self { if manager.attributes.outputType != cameraOutputType { manager.attributes.outputType = cameraOutputType }; return self }
     ...
     */
    func setCameraPosition(_ cameraPosition: CameraPosition) -> Self { if manager.attributes.cameraPosition != cameraPosition { manager.attributes.cameraPosition = cameraPosition }; return self }
     ...
     */
    func setAudioAvailability(_ isAvailable: Bool) -> Self { if manager.attributes.isAudioSourceAvailable != isAvailable { manager.attributes.isAudioSourceAvailable = isAvailable }; return self }
     ...
     */
    func setLightMode(_ lightMode: CameraLightMode) -> Self { if manager.attributes.lightMode != lightMode { manager.attributes.lightMode = lightMode }; return self }
    ...
    /**
     ...
     If you set a frame rate that exceeds the camera's capabilities, the library will automatically set the closest possible value and show you which value has been set (``CustomCameraScreen/frameRate``).
     */
     ...
     */
    func setMirrorOutput(_ shouldMirror: Bool) -> Self { if manager.attributes.mirrorOutput != shouldMirror { manager.attributes.mirrorOutput = shouldMirror }; return self }

    /**
     Changes the initial grid visibility setting.
     */
    func setGridVisibility(_ shouldShowGrid: Bool) -> Self { if manager.attributes.isGridVisible != shouldShowGrid { manager.attributes.isGridVisible = shouldShowGrid }; return self }
...
internal extension CustomCameraView {
    /**
     Indicates how the CustomCameraView can be closed.
        ...
        var body: some View {
            ...
            .fullScreenCover(isPresented: $isSheetPresented) {
                CustomCameraView()
                    ...
                    .setCloseCustomCameraAction { isSheetPresented = false }
     ...
     ```
     */
    func setCloseCustomCameraAction(_ action: @escaping () -> ()) -> Self { config.closeCustomCameraAction = action; return self }

    /**
     Defines action that is called when an image is captured.

     CustomCameraViewController can be used to perform additional actions related to CustomCameraView, such as closing CustomCameraView or returning to the camera screen.
     ...
     ```
     */
    func onImageCaptured(_ action: @escaping (UIImage, CustomCameraView.Controller) -> ()) -> Self { config.imageCapturedAction = action; return self }
    ...
    /**
     ...
     ```
     */
    func onVideoCaptured(_ action: @escaping (URL, CustomCameraView.Controller) -> ()) -> Self { config.videoCapturedAction = action; return self }

    /**
     Defines action that is called when multiple media items are accepted from the preview screen.

     When set, this batch callback is preferred over repeatedly invoking ``onImageCaptured(_:)`` / ``onVideoCaptured(_:)``.
     */
    func onMediaBatchCaptured(_ action: @escaping ([CustomCameraMedia], CustomCameraView.Controller) -> ()) -> Self {
        config.mediaBatchCapturedAction = action
        return self
    }
}
...
internal extension CustomCameraView {
    /**
     ...
     See ``CustomCameraApplicationDelegate`` for more details.
     ...
     ```
     */
    func lockCameraInPortraitOrientation(_ appDelegate: CustomCameraApplicationDelegate.Type) -> Self { config.appDelegate = appDelegate; manager.attributes.orientationLocked = true; return self }
